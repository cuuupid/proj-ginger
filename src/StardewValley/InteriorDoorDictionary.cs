...
namespace StardewValley
{
	public class InteriorDoorDictionary : NetPointDictionary<bool, InteriorDoor>
	{
		...
		{
			if (!location.isOutdoors && location.Map.Properties.TryGetValue("Doors", out var value) && value != null)
			{
		...
		{
			if ((bool)location.isOutdoors || !location.Map.Properties.TryGetValue("Doors", out var value) || value == null)
			{
				return;
			}
			string[] array = value.ToString().Split(' ');
			for (int i = 0; i < array.Length; i += 4)
			{
				...
				{
					InteriorDoor interiorDoor = base.FieldDict[point];
					interiorDoor.Location = location;
					interiorDoor.Position = point;
