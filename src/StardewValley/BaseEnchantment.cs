	...
	public class BaseEnchantment : INetObject<NetFields>
	{
		...
		public static BaseEnchantment GetEnchantmentFromItem(Item base_item, Item item)
		{
			...
			if (Utility.IsNormalObjectAtParentSheetIndex(item, 74))
			{
				...
				return Utility.GetRandom(GetAvailableEnchantmentsForItem(base_item as Tool), random);
			}
			return null;
		}
		...
		public static List<BaseEnchantment> GetAvailableEnchantmentsForItem(Tool item)
		{
			...
			foreach (string previousEnchantment in item.previousEnchantments)
			{
				if (list.Count > 1)
				{
					...
					{
						if (list[i].GetName() == previousEnchantment)
						{
							list.RemoveAt(i);
							break;
						}
		...
		{
			BaseEnchantment baseEnchantment = Activator.CreateInstance(GetType()) as BaseEnchantment;
			baseEnchantment.level.Value = level.Value;
