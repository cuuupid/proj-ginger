	...
	public class Debris : INetObject<NetFields>
	{
			...
			: this(objectIndex, 1, debrisOrigin, playerPosition)
		{
			...
			if (Game1.player.speed >= 5)
			{
				...
				{
					chunk.xVelocity.Value *= ((Game1.player.FacingDirection == 1 || Game1.player.FacingDirection == 3) ? 1 : 1);
				}
			}
		...
		public virtual bool collect(Farmer farmer, Chunk chunk = null)
		{
			...
			int num = (((DebrisType)debrisType == DebrisType.ARCHAEOLOGY || (DebrisType)debrisType == DebrisType.OBJECT) ? chunk.debrisType : (chunk.debrisType - chunk.debrisType % 2));
			...
			else if ((DebrisType)debrisType != 0 || num != 8)
			{
				if (num <= -10000)
				{
					if (!farmer.addItemToInventoryBool(new MeleeWeapon(num)))
					{
						return false;
					}
				}
				...
				{
					if (!farmer.addItemToInventoryBool(new Object(Vector2.Zero, -num)))
					{
						return false;
					}
				}
				else if (!farmer.addItemToInventoryBool((num == 93 || num == 94) ? new Torch(Vector2.Zero, 1, num) : new Object(Vector2.Zero, num, 1)
				{
					Quality = itemQuality
				}))
				{
			...
			: this()
		{
			...
			{
				chunks.Add(new Chunk(debrisOrigin, (float)Game1.recentMultiplayerRandom.Next(num3, num4) / 40f, (float)Game1.recentMultiplayerRandom.Next(num, num2) / 40f, Game1.recentMultiplayerRandom.Next(debrisType, debrisType + 2)));
			}
		}
		...
		private bool playerInRange(Vector2 position, Farmer farmer)
		{
			...
			if (Math.Abs(position.X + 32f - (float)farmer.getStandingX()) <= (float)appliedMagneticRadius)
			{
				return Math.Abs(position.Y + 32f - (float)farmer.getStandingY()) <= (float)appliedMagneticRadius;
			}
			return false;
		}
		...
		private Farmer findBestPlayer(GameLocation location)
		{
			...
			{
				if ((farmer2.UniqueMultiplayerID != (long)DroppedByPlayerID || farmer == null) && playerInRange(vector, farmer2))
				{
					float num2 = (farmer2.Position - vector).LengthSquared();
					if (num2 < num || (farmer != null && farmer.UniqueMultiplayerID == (long)DroppedByPlayerID))
					{
						farmer = farmer2;
						num = num2;
					}
		...
		public bool updateChunks(GameTime time, GameLocation location)
		{
			...
			DebrisType value = debrisType.Value;
			timeSinceDoneBouncing += time.ElapsedGameTime.Milliseconds;
			if (timeSinceDoneBouncing >= (floppingFish ? 2500f : ((value == DebrisType.SPRITECHUNKS || value == DebrisType.NUMBERS) ? 1800f : 600f)))
			{
				if (value == DebrisType.LETTERS || value == DebrisType.NUMBERS || value == DebrisType.SQUARES || value == DebrisType.SPRITECHUNKS || (value == DebrisType.CHUNKS && chunks[0].debrisType - chunks[0].debrisType % 2 != 8))
				{
					return true;
				}
				if (value == DebrisType.ARCHAEOLOGY || value == DebrisType.OBJECT || value == DebrisType.RESOURCE || value == DebrisType.CHUNKS)
				{
			...
			for (int num = chunks.Count - 1; num >= 0; num--)
			{
				...
				if (chunk.alpha > 0.1f && (value == DebrisType.SPRITECHUNKS || value == DebrisType.NUMBERS) && timeSinceDoneBouncing > 600f)
				{
				...
				else
				{
					...
					if (flag2)
					{
						...
						case DebrisType.OBJECT:
							...
							flag2 = ((chunk.debrisType >= 0) ? farmer.couldInventoryAcceptThisObject(chunk.debrisType, 1, itemQuality) : farmer.couldInventoryAcceptThisItem(new Object(Vector2.Zero, chunk.debrisType * -1)));
							...
							if (Math.Abs(chunk.position.X + 32f - (float)player.Value.getStandingX()) <= 64f && Math.Abs(chunk.position.Y + 32f - (float)player.Value.getStandingY()) <= 64f)
							{
								...
								if (collect(player, chunk))
								{
									if (Game1.debrisSoundInterval <= 0f)
									{
										...
										if ((item == null || (int)item.parentSheetIndex != 73) && chunk.debrisType != 73)
										{
					...
					{
						if (value == DebrisType.NUMBERS && toHover != null)
						{
						...
						if (value == DebrisType.SQUARES && chunk.position.Y < (float)(chunkFinalYLevel - 96) && Game1.random.NextDouble() < 0.1)
						{
						...
						if (value != DebrisType.SQUARES && chunk.bounces <= (floppingFish ? 65 : 2))
						{
						...
						{
							Point p = new Point((int)chunk.position.X / 64, chunkFinalYLevel / 64);
							if (Game1.currentLocation is IslandNorth && (value == DebrisType.ARCHAEOLOGY || value == DebrisType.OBJECT || value == DebrisType.RESOURCE || value == DebrisType.CHUNKS) && Game1.currentLocation.isTileOnMap(p.X, p.Y) && Game1.currentLocation.getTileIndexAt(p, "Back") == -1)
							{
								chunkFinalYLevel += 48;
							}
							if (value != DebrisType.LETTERS && value != DebrisType.NUMBERS && value != DebrisType.SPRITECHUNKS && (value != 0 || chunk.debrisType - chunk.debrisType % 2 == 8) && shouldControlThis(location))
							{
							...
							if (value != DebrisType.LETTERS && value != DebrisType.SPRITECHUNKS && value != DebrisType.NUMBERS && location.doesTileSinkDebris((int)chunkTile.X, (int)chunkTile.Y, value))
							{
								flag3 = location.sinkDebris(this, chunkTile, chunk.position);
							}
						}
						int num2 = (int)((chunk.position.X + 32f) / 64f);
						int num3 = (int)((chunk.position.Y + 32f) / 64f);
						if ((!chunk.hitWall && location.Map.GetLayer("Buildings").Tiles[num2, num3] != null && location.doesTileHaveProperty(num2, num3, "Passable", "Buildings") == null) || location.Map.GetLayer("Back").Tiles[num2, num3] == null)
						{
