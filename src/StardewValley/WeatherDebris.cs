...
namespace StardewValley
{
	...
	public class WeatherDebris
	{
		...
		{
			Initialize(position, which, rotationVelocity, dx, dy);
		}
		...
		public void update(bool slow)
		{
			...
			{
				int num = 16;
				if (position.X + 64f < (float)(Game1.viewport.X - num))
				{
					position.X = Game1.viewport.X + Game1.viewport.Width;
					position.Y = Game1.viewport.Y + Game1.random.Next(-64, Game1.viewport.Height);
				}
				else if (position.X > (float)(Game1.viewport.X + Game1.viewport.Width + num))
				{
					position.X = Game1.viewport.X - 64;
					position.Y = Game1.viewport.Y + Game1.random.Next(-64, Game1.viewport.Height);
				}
				if (position.Y + 64f < (float)(Game1.viewport.Y - num))
				{
					position.X = Game1.viewport.X + Game1.random.Next(-64, Game1.viewport.Width);
					position.Y = Game1.viewport.Y + Game1.viewport.Height;
				}
				else if (position.Y > (float)(Game1.viewport.Y + Game1.viewport.Height + num))
				{
					position.X = Game1.viewport.X + Game1.random.Next(-64, Game1.viewport.Width);
					position.Y = Game1.viewport.Y - 64;
		...
		{
			b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, position), sourceRect, Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1E-06f);
		}
	}
}
