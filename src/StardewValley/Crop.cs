...
namespace StardewValley
{
	public class Crop : INetObject<NetFields>
	{
		...
		public bool ReadyToHarvest
		{
			get
			{
				if ((int)currentPhase >= phaseDays.Count - 1)
				{
					if ((bool)fullyGrown)
					{
						return (int)dayOfCurrentPhase <= 0;
					}
					return true;
				}
				return false;
			...
			: this()
		{
			...
			{
				string[] array = dictionary[seedIndex].Split('/');
				string[] array2 = array[0].Split(' ');
				for (int i = 0; i < array2.Length; i++)
				{
					phaseDays.Add(Convert.ToInt32(array2[i]));
				}
				phaseDays.Add(99999);
				string[] array3 = array[1].Split(' ');
				for (int j = 0; j < array3.Length; j++)
				{
					seasonsToGrowIn.Add(array3[j]);
				}
				rowInSpriteSheet.Value = Convert.ToInt32(array[2]);
				...
				indexOfHarvest.Value = Convert.ToInt32(array[3]);
				regrowAfterHarvest.Value = Convert.ToInt32(array[4]);
				harvestMethod.Value = Convert.ToInt32(array[5]);
				ResetCropYield();
				raisedSeeds.Value = Convert.ToBoolean(array[7]);
				string[] array4 = array[8].Split(' ');
				if (array4.Length != 0 && array4[0].Equals("true"))
				{
					List<Color> list = new List<Color>();
					for (int k = 1; k < array4.Length; k += 3)
					{
						list.Add(new Color(Convert.ToByte(array4[k]), Convert.ToByte(array4[k + 1]), Convert.ToByte(array4[k + 2])));
					}
					Random random = new Random(tileX * 1000 + tileY + Game1.dayOfMonth);
					tintColor.Value = list[random.Next(list.Count)];
		...
		public virtual void InferSeedIndex()
		{
			...
			foreach (int key in dictionary.Keys)
			{
				...
				if (Convert.ToInt32(array[3]) == indexOfHarvest.Value)
				{
		...
		public virtual void ResetCropYield()
		{
			...
			num = (((int)rowInSpriteSheet != 23) ? netSeedIndex.Value : whichForageCrop.Value);
			if (dictionary.ContainsKey(num))
			{
				string[] array = dictionary[num].Split('/');
				string[] array2 = array[6].Split(' ');
				if (array2.Length != 0 && array2[0].Equals("true"))
				{
					minHarvest.Value = Convert.ToInt32(array2[1]);
					maxHarvest.Value = Convert.ToInt32(array2[2]);
					maxHarvestIncreasePerFarmingLevel.Value = Convert.ToInt32(array2[3]);
					chanceForExtraCrops.Value = Convert.ToDouble(array2[4]);
				}
				else
				{
					minHarvest.Value = 1;
		...
		public virtual bool hitWithHoe(int xTile, int yTile, GameLocation location, HoeDirt dirt)
		{
			if ((bool)forageCrop && (int)whichForageCrop == 2)
			{
				...
				Game1.createItemDebris(@object.getOne(), new Vector2(xTile * 64 + 32, yTile * 64 + 32), -1);
				return true;
			}
			return false;
		}
		...
		public virtual bool harvest(int xTile, int yTile, HoeDirt soil, JunimoHarvester junimoHarvester = null)
		{
			...
			if ((bool)forageCrop)
			{
				...
				if (Game1.player.addItemToInventoryBool(@object))
				{
					...
					if ((int)regrowAfterHarvest == -1)
					{
						Game1.multiplayer.broadcastSprites(Game1.currentLocation, new TemporaryAnimatedSprite(17, new Vector2(vector.X * 64f, vector.Y * 64f), Color.White, 7, random.NextDouble() < 0.5, 125f));
						Game1.multiplayer.broadcastSprites(Game1.currentLocation, new TemporaryAnimatedSprite(14, new Vector2(vector.X * 64f, vector.Y * 64f), Color.White, 7, random.NextDouble() < 0.5, 50f));
					}
			...
			else if ((int)currentPhase >= phaseDays.Count - 1 && (!fullyGrown || (int)dayOfCurrentPhase <= 0))
			{
				...
				Random random2 = new Random(xTile * 7 + yTile * 11 + (int)Game1.stats.DaysPlayed + (int)Game1.uniqueIDForThisGame);
				...
				if ((int)minHarvest > 1 || (int)maxHarvest > 1)
				{
					...
					{
						num5 = Game1.player.FarmingLevel / (int)maxHarvestIncreasePerFarmingLevel;
					}
					num = random2.Next(minHarvest, Math.Max((int)minHarvest + 1, (int)maxHarvest + 1 + num5));
				}
				if ((double)chanceForExtraCrops > 0.0)
				{
					while (random2.NextDouble() < Math.Min(0.9, chanceForExtraCrops))
					{
						num++;
					}
				}
				...
				Object object2 = (programColored ? new ColoredObject(indexOfHarvest, 1, tintColor)
				{
					Quality = quality
				} : new Object(indexOfHarvest, 1, isRecipe: false, -1, quality));
				if ((int)harvestMethod == 1)
				{
					...
					{
						Game1.createItemDebris(object2.getOne(), new Vector2(xTile * 64 + 32, yTile * 64 + 32), -1);
					}
					flag = true;
				}
				...
				if (flag)
				{
					...
					int num6 = Convert.ToInt32(Game1.objectInformation[indexOfHarvest].Split('/')[1]);
					object2 = (programColored ? new ColoredObject(indexOfHarvest, 1, tintColor) : new Object(indexOfHarvest, 1));
					...
					if ((int)indexOfHarvest == 262 && random2.NextDouble() < 0.4)
					{
						Object object3 = new Object(178, 1);
						if (junimoHarvester == null)
						{
							Game1.createItemDebris(object3.getOne(), new Vector2(xTile * 64 + 32, yTile * 64 + 32), -1);
						}
						else
						{
					...
					else if ((int)indexOfHarvest == 771)
					{
						...
						{
							Object object4 = new Object(770, 1);
							if (junimoHarvester == null)
							{
								Game1.createItemDebris(object4.getOne(), new Vector2(xTile * 64 + 32, yTile * 64 + 32), -1);
							}
							else
							{
		...
		public virtual Rectangle getSourceRect(int number)
		{
			...
			int num = rowInSpriteSheet;
			if ((int)indexOfHarvest == 771)
			{
				if (Game1.currentSeason == "fall")
				{
					num = (int)rowInSpriteSheet + 1;
				}
				else if (Game1.currentSeason == "winter")
				{
					num = (int)rowInSpriteSheet + 2;
				}
			}
			return new Rectangle(Math.Min(240, ((!fullyGrown) ? ((int)(((int)phaseToShow != -1) ? phaseToShow : currentPhase) + (((int)(((int)phaseToShow != -1) ? phaseToShow : currentPhase) == 0 && number % 2 == 0) ? (-1) : 0) + 1) : (((int)dayOfCurrentPhase <= 0) ? 6 : 7)) * 16 + ((num % 2 != 0) ? 128 : 0)), num / 2 * 16 * 2, 16, 32);
		}
		...
		{
			return sourceRect == Rectangle.Empty;
		}

		public virtual void newDay(int state, int fertilizer, int xTile, int yTile, GameLocation environment)
		{
				...
				if (environment is Farm && (int)currentPhase == phaseDays.Count - 1 && ((int)indexOfHarvest == 276 || (int)indexOfHarvest == 190 || (int)indexOfHarvest == 254) && OneTimeRandom.GetDouble(Game1.uniqueIDForThisGame, Game1.stats.DaysPlayed, (ulong)xTile, (ulong)yTile) < 0.01)
				{
					for (int i = xTile - 1; i <= xTile + 1; i++)
					{
						for (int j = yTile - 1; j <= yTile + 1; j++)
						{
							...
							if (!environment.terrainFeatures.ContainsKey(key) || !(environment.terrainFeatures[key] is HoeDirt) || (environment.terrainFeatures[key] as HoeDirt).crop == null || (environment.terrainFeatures[key] as HoeDirt).crop.indexOfHarvest != indexOfHarvest)
							{
								return;
							}
						}
			...
			if ((!fullyGrown || (int)dayOfCurrentPhase <= 0) && (int)currentPhase >= phaseDays.Count - 1 && (int)rowInSpriteSheet == 23)
			{
				...
				if (environment.objects.ContainsKey(vector))
				{
					if (environment.objects[vector] is IndoorPot)
					{
						(environment.objects[vector] as IndoorPot).heldObject.Value = new Object(vector, getRandomWildCropForSeason(season), 1);
						(environment.objects[vector] as IndoorPot).hoeDirt.Value.crop = null;
					}
					else
					{
				...
				if (!environment.objects.ContainsKey(vector))
				{
					environment.objects.Add(vector, new Object(vector, getRandomWildCropForSeason(season), 1)
					{
						IsSpawnedObject = true,
						CanBeGrabbed = true
					});
				}
				if (environment.terrainFeatures.ContainsKey(vector) && environment.terrainFeatures[vector] != null && environment.terrainFeatures[vector] is HoeDirt)
				{
		...
		public virtual void draw(SpriteBatch b, Vector2 tileLocation, Color toTint, float rotation)
		{
			...
			b.Draw(Game1.cropSpriteSheet, position, sourceRect, toTint, rotation, origin, 4f, effects, layerDepth);
			Color value = tintColor.Value;
			if (value.PackedValue != Color.White.PackedValue && (int)currentPhase == phaseDays.Count - 1 && !dead)
			{
				b.Draw(Game1.cropSpriteSheet, position, coloredSourceRect, value, rotation, origin, 4f, effects, coloredLayerDepth);
			}
		}
