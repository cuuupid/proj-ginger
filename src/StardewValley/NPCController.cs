	...
	public class NPCController
	{
		...
		private bool setMoving(bool newTarget)
		{
			if (puppet != null && pathIndex < path.Count)
			{
				...
				puppet.Halt();
				puppet.faceDirection(direction);
				if (CurrentPathX != 0 && CurrentPathY != 0)
				{
					pauseTime = CurrentPathY;
					direction = CurrentPathX % 4;
					puppet.faceDirection(direction);
					return true;
				}
		...
		public bool update(GameTime time, GameLocation location, List<NPCController> allControllers)
		{
			...
			foreach (NPCController allController in allControllers)
			{
				...
				if (allController.puppet.Equals(puppet))
				{
					flag = true;
				}
				if (allController.puppet.FacingDirection == puppet.FacingDirection && !allController.puppet.Equals(puppet) && allController.puppet.GetBoundingBox().Intersects(puppet.nextPosition(puppet.FacingDirection)))
				{
					if (flag)
					{
						break;
