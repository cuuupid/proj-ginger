		...
		public bool update(GameTime time, Event e)
		{
			...
			if (steamTimer <= 0)
			{
				...
				e.aboveMapSprites.Add(new TemporaryAnimatedSprite
				{
					texture = tempText,
					local = true,
					position = new Vector2(topLeftX + width / 2, topLeftY + height / 2) + new Vector2(-64 + num, 64f),
					initialPosition = new Vector2(topLeftX + width / 2, topLeftY + height / 2) + new Vector2(-64 + num, 64f),
			...
			if (buddy != null)
			{
				...
				case 2:
				{
					...
					{
						if (e.aboveMapSprites[j].id == 777f)
						{
				...
				case 5:
					...
					if (buddyTimer <= 0)
					{
						...
						for (int i = 0; i < 30; i++)
						{
							Vector2 vector = Utility.getRandomPositionInThisRectangle(new Rectangle(-8, -8, 27, 32), Game1.random) * 4f;
							float x = Utility.Lerp(-2f, 2f, (float)Game1.random.NextDouble());
							e.aboveMapSprites.Add(new TemporaryAnimatedSprite
							{
								texture = tempText,
								local = true,
								position = buddy.position + vector,
								initialPosition = buddy.position + vector,
