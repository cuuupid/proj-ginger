...
namespace StardewValley
{
	public class MoneyMadeScreen
	{
		...
		public MoneyMadeScreen(List<Object> shippingItems, int timeOfDay)
		{
			...
			int randomItemFromSeason = Utility.getRandomItemFromSeason(Game1.currentSeason, 0, forQuest: false);
			...
			foreach (Object shippingItem in shippingItems)
			{
				...
				if (shippingItem.ParentSheetIndex == num)
				{
		...
		public void draw(GameTime gametime)
		{
			...
			int num = Game1.graphics.GraphicsDevice.Viewport.GetTitleSafeArea().Height - 128;
			int num2 = Game1.graphics.GraphicsDevice.Viewport.GetTitleSafeArea().X + Game1.graphics.GraphicsDevice.Viewport.Width / 2 - (int)((float)((shippingItems.Count / (num / 64 - 4) + 1) * 64) * 3f);
			int num3 = Game1.graphics.GraphicsDevice.Viewport.GetTitleSafeArea().Y + 64;
			int num4 = (int)((float)((shippingItems.Count / (num / 64 - 4) + 1) * 64) * 6f);
			Game1.drawDialogueBox(num2, num3, num4, num, speaker: false, drawOnlyBox: true);
			int num5 = num - 192;
			Point point = new Point(num2 + 64, num3 + 64);
			for (int i = 0; i < currentItemIndex; i++)
			{
				Game1.spriteBatch.Draw(Game1.objectSpriteSheet, new Vector2(point.X + i * 64 / (num5 - 128) * 64 * 4 + 32, i * 64 % (num5 - 128) - i * 64 % (num5 - 128) % 64 + Game1.graphics.GraphicsDevice.Viewport.GetTitleSafeArea().Y + 192 + 32), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, shippingItems.Keys.ElementAt(i).index), Color.White, 0f, new Vector2(32f, 32f), shippingItems.Keys.ElementAt(i).name.Contains("=") ? starScale : 1f, SpriteEffects.None, 0.999999f);
				Game1.spriteBatch.DrawString(Game1.dialogueFont, "x" + shippingItems[shippingItems.Keys.ElementAt(i)] + " : " + shippingItems.Keys.ElementAt(i).price * shippingItems[shippingItems.Keys.ElementAt(i)] + "g", new Vector2(point.X + i * 64 / (num5 - 128) * 64 * 4 + 64, (float)(i * 64 % (num5 - 128) - i * 64 % (num5 - 128) % 64 + 32) - Game1.dialogueFont.MeasureString("9").Y / 2f + (float)Game1.graphics.GraphicsDevice.Viewport.GetTitleSafeArea().Y + 192f), Game1.textColor);
			}
			if (complete)
			{
				Game1.spriteBatch.DrawString(Game1.dialogueFont, Game1.content.LoadString("Strings\\StringsFromCSFiles:MoneyMadeScreen.cs.3854", total), new Vector2((float)(num2 + num4 - 64) - Game1.dialogueFont.MeasureString("Total: " + total).X, Game1.graphics.GraphicsDevice.Viewport.GetTitleSafeArea().Bottom - 160), Game1.textColor);
			}
		}
	}
}
