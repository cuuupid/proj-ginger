...
namespace StardewValley
{
	public class InteriorDoor : NetField<bool, InteriorDoor>
	{
		...
		public void ResetLocalState()
		{
			...
			Layer layer2 = Location.Map.GetLayer("Back");
			if (Tile == null)
			{
			...
			if (Tile.Properties.TryGetValue("Action", out var propertyValue) && propertyValue != null && propertyValue.ToString().Contains("Door") && propertyValue.ToString().Split(' ').Length > 1 && layer2.Tiles[location] != null && !layer2.Tiles[location].Properties.ContainsKey("TouchAction"))
			{
		...
		private void closeDoorTiles()
		{
			...
			{
				map.GetLayer("Buildings").Tiles[location] = Tile;
				Location.removeTileProperty(Position.X, Position.Y, "Back", "TemporaryBarrier");
				location.Y--;
				map.GetLayer("Front").Tiles[location] = new StaticTile(map.GetLayer("Front"), Tile.TileSheet, BlendMode.Alpha, Tile.TileIndex - Tile.TileSheet.SheetWidth);
				location.Y--;
				map.GetLayer("Front").Tiles[location] = new StaticTile(map.GetLayer("Front"), Tile.TileSheet, BlendMode.Alpha, Tile.TileIndex - Tile.TileSheet.SheetWidth * 2);
			}
		}
