...
namespace StardewValley
{
	internal class ReadyCheck : INetObject<NetFields>
	{
		...
		public void SetRequiredFarmers(IEnumerable<Farmer> required_farmers)
		{
			...
			foreach (Farmer required_farmer in required_farmers)
			{
				requiredPlayers.Add(required_farmer);
			}
		}
		...
		{
			IEnumerable<Farmer> enumerable = GetRequiredPlayers();
			foreach (Farmer item in enumerable)
			{
				if (!farmerSet.Contains(item) && !Game1.multiplayer.isDisconnecting(item))
				{
					return false;
				}
			}
		...
		public void Update()
		{
			if (readyPlayers.Contains(Game1.player) && !setPlayers.Contains(Game1.player) && !(Game1.activeClickableMenu is SaveGameMenu) && !(Game1.activeClickableMenu is ShippingMenu) && (!(Game1.activeClickableMenu is ReadyCheckDialog readyCheckDialog) || readyCheckDialog.checkName != Name))
			{
