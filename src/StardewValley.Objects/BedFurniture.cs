...
namespace StardewValley.Objects
{
	public class BedFurniture : Furniture
	{
		...
		public static BedFurniture GetBedAtTile(GameLocation location, int x, int y)
		{
			...
			{
				if (Utility.doesRectangleIntersectTile(item.getBoundingBox(item.tileLocation), x, y) && item is BedFurniture)
				{
					return item as BedFurniture;
				}
			}
			return null;
		}
		...
		public static void ApplyWakeUpPosition(Farmer who)
		{
			...
			{
				GameLocation locationFromName = Game1.getLocationFromName(who.lastSleepLocation);
			...
			{
				GameLocation gameLocation = (who.currentLocation = Game1.getLocationFromName(who.homeLocation.Value));
				who.Position = Utility.PointToVector2((gameLocation as FarmHouse).GetPlayerBedSpot()) * 64f;
				ShiftPositionForBed(who);
			}
			if (who == Game1.player)
			{
		...
		{
			BedFurniture bedAtTile = GetBedAtTile(who.currentLocation, (int)(who.position.X / 64f), (int)(who.position.Y / 64f));
			if (bedAtTile != null)
			{
		...
		{
			Rectangle rectangle = getBoundingBox(tileLocation);
			if (bedType == BedType.Double)
			{
				bedTileOffset = 1;
			}
