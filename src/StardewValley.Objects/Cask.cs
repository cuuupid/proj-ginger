...
namespace StardewValley.Objects
{
	public class Cask : Object
	{
		...
		public override bool performObjectDropInAction(Item dropIn, bool probe, Farmer who)
		{
			...
			if (dropIn is Object @object && @object.Quality >= 4)
			{
				return false;
			}
			bool flag = false;
			float num = 1f;
			num = GetAgingMultiplierForItem(dropIn);
			if (num > 0f)
			{
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			...
			{
				Vector2 vector = (((int)minutesUntilReady > 0) ? new Vector2(Math.Abs(scale.X - 5f), Math.Abs(scale.Y - 5f)) : Vector2.Zero);
				vector *= 4f;
				...
				spriteBatch.Draw(Game1.mouseCursors, destinationRectangle, ((int)heldObject.Value.quality < 4) ? new Rectangle(338 + ((int)heldObject.Value.quality - 1) * 8, 400, 8, 8) : new Rectangle(346, 392, 8, 8), Color.White * 0.95f, 0f, Vector2.Zero, SpriteEffects.None, (float)((y + 1) * 64) / 10000f);
			}
		}
	}
}
