...
namespace StardewValley.Objects
{
	public class Ring : Item
	{
		...
		public Ring(int which)
			: this()
		{
			string[] array = Game1.objectInformation[which].Split('/');
			base.Category = -96;
			Name = array[0];
		...
		public virtual void update(GameTime time, GameLocation environment, Farmer who)
		{
			if (_lightSourceID.HasValue)
			{
				...
				{
					zero += (Vector2)who.drawOffset;
				}
				environment.repositionLightSource(_lightSourceID.Value, new Vector2(who.Position.X + 21f, who.Position.Y) + zero);
				if (!environment.isOutdoors && !(environment is MineShaft) && !(environment is VolcanoDungeon))
				{
					LightSource lightSource = environment.getLightSource(_lightSourceID.Value);
					if (lightSource != null)
					{
		...
		protected virtual bool loadDisplayFields()
		{
			...
			{
				string[] array = Game1.objectInformation[indexInTileSheet].Split('/');
				displayName = array[4];
				description = array[5];
				return true;
			}
