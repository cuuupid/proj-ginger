...
namespace StardewValley.Objects
{
	public class IndoorPot : Object
	{
		...
		public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
		{
			if (who != null)
			{
				if (justCheckingForActivity)
				{
					...
					if (bush.Value != null && (int)bush.Value.overrideSeason != -1)
					{
						season = Utility.getSeasonNameFromNumber(bush.Value.overrideSeason);
					}
					if (!hoeDirt.Value.readyForHarvest() && heldObject.Value == null)
					{
						if (bush.Value != null)
						{
							return bush.Value.inBloom(season, Game1.dayOfMonth);
						}
						return false;
					}
					return true;
				...
				if (heldObject.Value != null)
				{
					bool flag = who.addItemToInventoryBool(heldObject.Value);
					if (flag)
					{
				...
				bool flag2 = hoeDirt.Value.performUseAction(tileLocation, who.currentLocation);
				if (flag2)
				{
					return flag2;
				}
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			...
			float num = 0.01f;
			spriteBatch.Draw(Game1.bigCraftableSpriteSheet, destinationRectangle, Object.getSourceRectForBigCraftable(showNextIndex ? (base.ParentSheetIndex + 1) : base.ParentSheetIndex), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, num);
			if ((int)hoeDirt.Value.fertilizer != 0)
			{
				Rectangle fertilizerSourceRect = hoeDirt.Value.GetFertilizerSourceRect(hoeDirt.Value.fertilizer);
				fertilizerSourceRect.Width = 13;
				fertilizerSourceRect.Height = 13;
				num += 0.001f;
				spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f + 4f, tileLocation.Y * 64f - 12f)), fertilizerSourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, num);
			}
			if (hoeDirt.Value.crop != null)
			{
				if (hoeDirt.Value.crop.isSourceRectEmpty())
				{
					hoeDirt.Value.crop.updateDrawMath(tileLocation);
				}
			...
			{
				num += 0.01f;
				heldObject.Value.draw(spriteBatch, x * 64, y * 64 - 48, num, 1f);
			}
			if (bush.Value != null)
			{
