...
namespace StardewValley.Objects
{
	public class ItemPedestal : Object
	{
		...
		public override void draw(SpriteBatch b, int x, int y, float alpha = 1f)
		{
			...
			b.Draw(Game1.bigCraftableSpriteSheet, Game1.GlobalToLocal(Game1.viewport, globalPosition), Object.getSourceRectForBigCraftable(pedestalType.Value), Color.White, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, Math.Max(0f, (globalPosition.Y - 2f) / 10000f));
			if (match.Value)
			{
				b.Draw(Game1.bigCraftableSpriteSheet, Game1.GlobalToLocal(Game1.viewport, globalPosition), Object.getSourceRectForBigCraftable(pedestalType.Value + 1), successColor.Value, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, Math.Max(0f, (globalPosition.Y - 1f) / 10000f));
			}
			if (heldObject.Value != null)
			{
