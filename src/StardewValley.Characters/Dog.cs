...
namespace StardewValley.Characters
{
	public class Dog : Pet
	{
		...
		public override void OnPetAnimationEvent(string animation_event)
		{
			...
			case "pant":
				if (base.CurrentBehavior == 50)
				{
					...
					for (int i = 0; i < num; i++)
					{
						list.Add(new FarmerSprite.AnimationFrame(24, 200, secondaryArm: false, flip: false, pantSound));
						list.Add(new FarmerSprite.AnimationFrame(25, 200, secondaryArm: false, flip: false));
					}
				...
				else if (base.CurrentBehavior == 2)
				{
					...
					int num2 = Game1.random.Next(7, 20);
					for (int j = 0; j < num2; j++)
					{
						list2.Add(new FarmerSprite.AnimationFrame(18, 200, secondaryArm: false, flip: false, pantSound));
						list2.Add(new FarmerSprite.AnimationFrame(19, 200));
					}
		...
		public override void RunState(GameTime time)
		{
			...
			{
				if (base.CurrentBehavior != 0 || !Game1.IsMasterGame || Sprite.CurrentAnimation != null || !(Game1.random.NextDouble() < 0.01) || forceUpdateTimer > 0)
				{
					return;
				}
		...
		public void wag()
		{
			int milliseconds = (withinPlayerThreshold(2) ? 120 : 200);
			wagging = true;
			Sprite.loop = false;
			List<FarmerSprite.AnimationFrame> list = new List<FarmerSprite.AnimationFrame>
			{
				new FarmerSprite.AnimationFrame(31, milliseconds, secondaryArm: false, flip: false),
				new FarmerSprite.AnimationFrame(23, milliseconds, secondaryArm: false, flip: false, hitGround)
			};
			int num = Game1.random.Next(2, 6);
			for (int i = 0; i < num; i++)
			{
				list.Add(new FarmerSprite.AnimationFrame(31, milliseconds, secondaryArm: false, flip: false));
				list.Add(new FarmerSprite.AnimationFrame(23, milliseconds, secondaryArm: false, flip: false, hitGround));
			}
			list.Add(new FarmerSprite.AnimationFrame(23, 2, secondaryArm: false, flip: false, doneWagging));
			Sprite.setCurrentAnimation(list);
		}
