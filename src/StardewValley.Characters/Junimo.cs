...
namespace StardewValley.Characters
{
	public class Junimo : NPC
	{
		...
		public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
		{
			...
			{
				Vector2 vector = Game1.GlobalToLocal(new Vector2(getStandingX(), (float)getStandingY() - 128f + (float)yJumpOffset));
				if (textAboveHeadStyle == 0)
				{
					vector += new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2));
				}
				SpriteText.drawStringWithScrollCenteredAt(b, textAboveHead, (int)vector.X, (int)vector.Y, "", textAboveHeadAlpha, textAboveHeadColor, 1, (float)(getTileY() * 64) / 10000f + 0.001f + (float)getTileX() / 10000f, !sayingGoodbye);
			}
		}
		...
		public override void update(GameTime time, GameLocation location)
		{
			...
			else
			{
				...
				if (!base.IsInvisible && farmerCloseCheckTimer <= 0 && controller == null && (float)alpha >= 1f && !stayPut && Game1.IsMasterGame)
				{
					...
					{
						Farmer farmer = Utility.isThereAFarmerWithinDistance(getTileLocation(), 5, base.currentLocation);
						if (farmer != null)
						{
							if ((bool)friendly && Vector2.Distance(base.Position, farmer.Position) > (float)(base.speed * 4))
							{
