...
namespace StardewValley.Characters
{
	public class Cat : Pet
	{
		...
		public override void OnPetAnimationEvent(string animation_event)
		{
			...
			else if (animation_event == "lick")
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					list.Add(new FarmerSprite.AnimationFrame(22, 150, secondaryArm: false, flip: false, lickSound));
					list.Add(new FarmerSprite.AnimationFrame(23, 150));
				}
				list.Add(new FarmerSprite.AnimationFrame(18, 1, secondaryArm: false, flip: false, base.hold));
				Sprite.loop = false;
		...
		{
			return "Animals\\cat" + ((whichBreed.Value == 0) ? "" : (whichBreed.Value.ToString() ?? ""));
		}
		...
		public override void RunState(GameTime time)
		{
			...
			{
				if (!Game1.IsMasterGame || Sprite.CurrentAnimation != null || !(Game1.random.NextDouble() < 0.01) || forceUpdateTimer > 0)
				{
					return;
				}
