...
namespace StardewValley.Characters
{
	public class TrashBear : NPC
	{
		...
		private void updateItemWanted()
		{
			...
			int num2 = 777111 + num;
			itemWantedIndex = Utility.getRandomPureSeasonalItem(Game1.currentSeason, num2);
			if (num <= 1)
			{
				return;
			}
			Random random = new Random((int)Game1.uniqueIDForThisGame + num2);
			int num3 = random.Next(CraftingRecipe.cookingRecipes.Count);
			int num4 = 0;
			foreach (string value in CraftingRecipe.cookingRecipes.Values)
			{
				if (num4 == num3)
				{
					...
					string text = array[2];
					text = text.Split(' ')[0];
					itemWantedIndex = Convert.ToInt32(text);
					break;
				}
				num4++;
			}
		...
		private void chew(Farmer who)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				Rectangle sourceRectForStandardTileSheet = Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, itemBeingEaten, 16, 16);
				...
				Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("Maps\\springobjects", sourceRectForStandardTileSheet, 400f, 1, 0, base.Position + new Vector2(64f, -48f), flicker: false, flipped: false, (float)getStandingY() / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (showWantBubbleTimer > 0)
			{
				...
				num -= 40f;
				b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocationPoint.X * 64 + 32, (float)(tileLocationPoint.Y * 64 - 96 - 48) + num)), new Rectangle(141, 465, 20, 24), Color.White * 0.75f, 0f, Vector2.Zero, 4f, SpriteEffects.None, num2 + 1E-06f);
				b.Draw(Game1.objectSpriteSheet, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocationPoint.X * 64 + 64 + 8, (float)(tileLocationPoint.Y * 64 - 64 - 32 - 8) + num)), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, itemWantedIndex, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, num2 + 1E-05f);
			}
		}
	}
}
