...
namespace StardewValley.Characters
{
	public class Pet : NPC
	{
		...
		{
			return "Animals\\dog" + ((whichBreed.Value == 0) ? "" : (whichBreed.Value.ToString() ?? ""));
		}
		...
		public void warpToFarmHouse(Farmer who)
		{
			...
			int i = 0;
			zero = new Vector2(Game1.random.Next(2, homeOfFarmer.map.Layers[0].LayerWidth - 3), Game1.random.Next(3, homeOfFarmer.map.Layers[0].LayerHeight - 5));
			List<Furniture> list = new List<Furniture>();
			foreach (Furniture item in homeOfFarmer.furniture)
			{
				if ((int)item.furniture_type == 12)
				{
					list.Add(item);
				}
			}
			BedFurniture playerBed = homeOfFarmer.GetPlayerBed();
			if (playerBed != null && !Game1.newDay && Game1.timeOfDay >= 2000 && ((this is Cat && Game1.random.NextDouble() <= 0.75) || Game1.random.NextDouble() <= 0.05000000074505806))
			{
				zero = Utility.PointToVector2(playerBed.GetBedSpot()) + new Vector2(-1f, 0f);
				Game1.warpCharacter(this, homeOfFarmer, zero);
				base.NetFields.CancelInterpolation();
				CurrentBehavior = 1;
				isSleepingOnFarmerBed.Value = true;
				foreach (Furniture item2 in homeOfFarmer.furniture)
				{
					if (item2 is BedFurniture && item2.getBoundingBox(item2.TileLocation).Intersects(GetBoundingBox()))
					{
						(item2 as BedFurniture).ReserveForNPC();
						break;
					}
				}
				UpdateSleepingOnBed();
			...
			{
				zero = Utility.PointToVector2(homeOfFarmer.getBedSpot()) + new Vector2(0f, 2f);
			}
			else if (Game1.random.NextDouble() <= 0.5)
			{
				Furniture random = Utility.GetRandom(list, Game1.random);
				if (random != null)
				{
					zero = new Vector2(random.boundingBox.Left / 64, random.boundingBox.Center.Y / 64);
				}
			}
			for (; i < 50; i++)
			{
				if (homeOfFarmer.canPetWarpHere(zero) && homeOfFarmer.isTileLocationTotallyClearAndPlaceable(zero) && homeOfFarmer.isTileLocationTotallyClearAndPlaceable(zero + new Vector2(1f, 0f)) && !homeOfFarmer.isTileOnWall((int)zero.X, (int)zero.Y))
				{
					break;
				}
				zero = new Vector2(Game1.random.Next(2, homeOfFarmer.map.Layers[0].LayerWidth - 3), Game1.random.Next(3, homeOfFarmer.map.Layers[0].LayerHeight - 4));
			}
			if (i < 50)
			{
				Game1.warpCharacter(this, homeOfFarmer, zero);
		...
		public void setAtFarmPosition()
		{
			...
			{
				bool flag = base.currentLocation is Farm;
				if (!Game1.isRaining)
				{
					faceDirection(2);
					Game1.warpCharacter(this, "Farm", (Game1.getLocationFromName("Farm") as Farm).GetPetStartLocation());
		...
		public override void behaviorOnFarmerPushing()
		{
			if (!(this is Dog) || (this as Dog).CurrentBehavior != 51)
			{
				...
				if (pushingTimer > 100)
				{
					Vector2 awayFromPlayerTrajectory = Utility.getAwayFromPlayerTrajectory(GetBoundingBox(), Game1.player);
		...
		public virtual void OnNewBehavior()
		{
			...
			case 1:
			{
				Sprite.loop = true;
				bool flag = false;
				flag = Game1.random.NextDouble() < 0.5;
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (base.IsEmoting)
			{
				...
				localPosition.X += 32f;
				localPosition.Y -= 96 + ((this is Dog) ? 16 : 0);
				b.Draw(Game1.emoteSpriteSheet, localPosition, new Rectangle(base.CurrentEmoteIndex * 16 % Game1.emoteSpriteSheet.Width, base.CurrentEmoteIndex * 16 / Game1.emoteSpriteSheet.Width * 16, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)getStandingY() / 10000f + 0.0001f);
			}
		}
		...
		public override bool withinPlayerThreshold(int threshold)
		{
			...
			foreach (Farmer farmer in base.currentLocation.farmers)
			{
				...
				if (Math.Abs(tileLocation.X - tileLocation2.X) <= (float)threshold && Math.Abs(tileLocation.Y - tileLocation2.Y) <= (float)threshold)
				{
					return true;
				}
			}
