	...
	public class Duggy : Monster
	{
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			if (Sprite.currentFrame < 4)
			{
				...
				if (!base.IsInvisible || boundingBox.Contains(base.Player.getStandingX(), base.Player.getStandingY()))
				{
					if (base.IsInvisible)
					{
			...
			{
				boundingBox.Inflate(-128, -128);
				base.currentLocation.isCollidingPosition(boundingBox, Game1.viewport, isFarmer: false, 8, glider: false, this);
			...
			if (Sprite.currentFrame >= 10)
			{
				...
				base.currentLocation.map.GetLayer("Back").Tiles[(int)tileLocation.X, (int)tileLocation.Y].TileIndex = 0;
				base.currentLocation.removeEverythingExceptCharactersFromThisTile((int)tileLocation.X, (int)tileLocation.Y);
				base.DamageToFarmer = 0;
			}
		}
	}
