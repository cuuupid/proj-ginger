	...
	public class BlueSquid : Monster
	{
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			if (projectileIntroTimer.Value <= 0f && projectileOutroTimer.Value <= 0f)
			{
				...
				{
					Vector2 vector = Vector2.Normalize(position.Value - findPlayer().position.Value) * Game1.random.Next(25, 50);
					vector.X *= -1f;
					if (canMoveTimer > 0f)
					{
					...
					{
						justThrust = true;
						Vector2 trajectory = Vector2.Normalize(position.Value - findPlayer().position.Value + new Vector2(Game1.random.Next(-64, 64))) * Game1.random.Next(25, 50);
						trajectory.X *= -1f;
			...
			if ((float)projectileIntroTimer > 0f)
			{
				...
				if ((float)projectileIntroTimer < 0f)
				{
					...
					DebuffingProjectile debuffingProjectile = new DebuffingProjectile(27, 8, 3, 4, 0f, velocityTowardPlayer.X, velocityTowardPlayer.Y, getStandingPosition() - new Vector2(32f, -squidYOffset), base.currentLocation, this);
