		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			if (yJumpOffset == 0)
			{
				if (Game1.random.NextDouble() < 0.01)
				{
					...
					foreach (Vector2 adjacentTileLocation in Utility.getAdjacentTileLocations(getTileLocation()))
					{
						if (base.currentLocation.objects.ContainsKey(adjacentTileLocation) && (base.currentLocation.objects[adjacentTileLocation].Name.Contains("Stone") || base.currentLocation.objects[adjacentTileLocation].Name.Contains("Twig")))
						{
							base.currentLocation.destroyObject(adjacentTileLocation, null);
						}
					}
					yJumpVelocity *= 2f;
				}
			...
			if (chargingFarmer)
			{
				...
				xVelocity += (0f - awayFromPlayerTrajectory.X) / 150f + ((Game1.random.NextDouble() < 0.01) ? ((float)Game1.random.Next(-50, 50) / 10f) : 0f);
				if (Math.Abs(xVelocity) > 5f)
				{
					xVelocity = Math.Sign(xVelocity) * 5;
				}
				yVelocity += (0f - awayFromPlayerTrajectory.Y) / 150f + ((Game1.random.NextDouble() < 0.01) ? ((float)Game1.random.Next(-50, 50) / 10f) : 0f);
