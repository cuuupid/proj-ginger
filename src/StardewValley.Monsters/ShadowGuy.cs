...
namespace StardewValley.Monsters
{
	public class ShadowGuy : Monster
	{
		...
		{
			int i = 0;
			Vector2 tileLocation = new Vector2(getTileLocation().X + (float)((Game1.random.NextDouble() < 0.5) ? Game1.random.Next(-5, -1) : Game1.random.Next(2, 6)), getTileLocation().Y + (float)((Game1.random.NextDouble() < 0.5) ? Game1.random.Next(-5, -1) : Game1.random.Next(2, 6)));
			for (; i < 6; i++)
			{
				if (base.currentLocation.isTileOnMap(tileLocation) && base.currentLocation.isTileLocationOpen(new Location((int)tileLocation.X, (int)tileLocation.Y)) && !base.currentLocation.isTileOccupiedForPlacement(tileLocation))
				{
					break;
				}
				tileLocation = new Vector2(getTileLocation().X + (float)((Game1.random.NextDouble() < 0.5) ? Game1.random.Next(-5, -1) : Game1.random.Next(2, 6)), getTileLocation().Y + (float)((Game1.random.NextDouble() < 0.5) ? Game1.random.Next(-5, -1) : Game1.random.Next(2, 6)));
			}
			if (i < 6)
			{
				teleporting = true;
				teleportationPath = Utility.GetPointsOnLine((int)getTileLocation().X, (int)getTileLocation().Y, (int)tileLocation.X, (int)tileLocation.Y, ignoreSwap: true).GetEnumerator();
				coolDown = 20;
			}
		}
