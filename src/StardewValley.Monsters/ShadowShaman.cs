		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			else if ((bool)casting)
			{
				...
				else
				{
					...
					if (monster != null)
					{
						int num2 = (isHardModeMonster ? 250 : 60);
						monster.Health = Math.Min(monster.MaxHealth, monster.Health + num2);
						base.currentLocation.playSound("healSound");
						Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 256, 64, 64), 40f, 8, 0, monster.Position + new Vector2(32f, 64f), flicker: false, flipped: false));
						base.currentLocation.debris.Add(new Debris(num2, new Vector2(monster.GetBoundingBox().Center.X, monster.GetBoundingBox().Center.Y), Color.Green, 1f, monster));
					}
				}
				casting.Value = false;
				coolDown = 1500;
