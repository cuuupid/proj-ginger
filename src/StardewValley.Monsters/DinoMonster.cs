...
namespace StardewValley.Monsters
{
	public class DinoMonster : Monster
	{
		...
		public override List<Item> getExtraDropItems()
		{
			...
			{
				list.Add(new Object(107, 1));
			}
			else
			{
				List<Item> list2 = new List<Item>();
				list2.Add(new Object(580, 1));
				list2.Add(new Object(583, 1));
				list2.Add(new Object(584, 1));
				list.Add(Utility.GetRandom(list2));
			}
			return list;
		}
			...
			else
			{
				...
				if (nextFireTime > 0)
				{
					...
					if (nextFireTime <= 0)
					{
						...
						float num2 = 0f;
						Vector2 startingPosition = new Vector2((float)GetBoundingBox().Center.X - 32f, (float)GetBoundingBox().Center.Y - 32f);
						...
						num2 += (float)Math.Sin((double)((float)totalFireTime / 1000f * 180f) * Math.PI / 180.0) * 25f;
						Vector2 vector = new Vector2((float)Math.Cos((double)num2 * Math.PI / 180.0), 0f - (float)Math.Sin((double)num2 * Math.PI / 180.0));
						vector *= 10f;
						BasicProjectile basicProjectile = new BasicProjectile(25, 10, 0, 1, (float)Math.PI / 16f, vector.X, vector.Y, startingPosition, "", "", explode: false, damagesMonsters: false, base.currentLocation, this);
