			...
			else if ((bool)throwing)
			{
				...
				if (Sprite.currentFrame == 24)
				{
					...
					if (isMage.Value)
					{
						...
						{
							base.currentLocation.projectiles.Add(new BasicProjectile(base.DamageToFarmer * 2, 9, 0, 4, 0f, velocityTowardPlayer.X, velocityTowardPlayer.Y, new Vector2(base.Position.X, base.Position.Y), "flameSpellHit", "flameSpell", explode: false, damagesMonsters: false, base.currentLocation, this));
						}
					}
					else
					{
						base.currentLocation.projectiles.Add(new BasicProjectile(base.DamageToFarmer, 4, 0, 0, (float)Math.PI / 16f, velocityTowardPlayer.X, velocityTowardPlayer.Y, new Vector2(base.Position.X, base.Position.Y), "skeletonHit", "skeletonStep", explode: false, damagesMonsters: false, base.currentLocation, this));
					}
				}
			}
