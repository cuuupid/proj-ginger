		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			if ((bool)isHardModeMonster)
			{
				...
				if (firingTimer <= 0f && numFireballsLeft > 0)
				{
					...
					BasicProjectile basicProjectile = new BasicProjectile(15, 10, 2, 4, 0f, velocityTowardPoint.X, velocityTowardPoint.Y, getStandingPosition() - new Vector2(32f, 0f), "", "", explode: true, damagesMonsters: false, base.currentLocation, this);
			...
			else if (withinPlayerThreshold() && lastFireball == 0f && Game1.random.NextDouble() < 0.01)
			{
				...
				Halt();
				fireballEvent.Fire();
				vector2 += fireballFired();
				Sprite.UpdateSourceRect();
				Vector2 velocityTowardPlayer = Utility.getVelocityTowardPlayer(Utility.Vector2ToPoint(getStandingPosition()), 8f, base.Player);
				BasicProjectile basicProjectile2 = new BasicProjectile(15, 10, 3, 4, 0f, velocityTowardPlayer.X, velocityTowardPlayer.Y, getStandingPosition() - new Vector2(32f, 0f), "", "", explode: true, damagesMonsters: false, base.currentLocation, this);
