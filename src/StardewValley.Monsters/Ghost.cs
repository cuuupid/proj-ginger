...
namespace StardewValley.Monsters
{
	public class Ghost : Monster
	{
		...
		public override List<Item> getExtraDropItems()
		{
			...
			{
				Object @object = new Object(875, 1)
				{
					specialItem = true
				};
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			if (variant.Value == GhostVariant.Putrid)
			{
				...
				else if (currentState.Value == 3)
				{
					...
					if (stateTimer <= 0f)
					{
						...
						BasicProjectile basicProjectile = new BasicProjectile(base.DamageToFarmer, 7, 0, 1, (float)Math.PI / 32f, vector2.X, vector2.Y, base.Position, "", "", explode: false, damagesMonsters: false, base.currentLocation, this);
			...
			int i = 0;
			Vector2 vector3 = new Vector2(base.Player.GetBoundingBox().Center.X / 64 + Game1.random.Next(-12, 12), base.Player.GetBoundingBox().Center.Y / 64 + Game1.random.Next(-12, 12));
			for (; i < 3; i++)
			{
				if (!(vector3.X >= (float)base.currentLocation.map.GetLayer("Back").LayerWidth) && !(vector3.Y >= (float)base.currentLocation.map.GetLayer("Back").LayerHeight) && !(vector3.X < 0f) && !(vector3.Y < 0f) && base.currentLocation.map.GetLayer("Back").Tiles[(int)vector3.X, (int)vector3.Y] != null && base.currentLocation.isTilePassable(new Location((int)vector3.X, (int)vector3.Y), Game1.viewport) && !vector3.Equals(new Vector2(base.Player.getStandingX() / 64, base.Player.getStandingY() / 64)))
				{
					break;
				}
				vector3 = new Vector2(base.Player.GetBoundingBox().Center.X / 64 + Game1.random.Next(-12, 12), base.Player.GetBoundingBox().Center.Y / 64 + Game1.random.Next(-12, 12));
			}
			if (i < 3)
			{
