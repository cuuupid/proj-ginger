...
namespace StardewValley.Minigames
{
	...
	public class Intro : IMinigame
	{
		...
		public void createBeginningOfLevel()
		{
			...
			roadsideObjects.Add(-1);
			roadsideObjects.Add(-1);
			busPosition = new Vector2(((float)(Game1.viewport.Width / 3) - 192f) / 2f, 240f);
		}
		...
		public void updateUpperClouds(GameTime time)
		{
			...
			if (Game1.random.NextDouble() < 0.0002)
			{
				...
				if (Game1.random.NextDouble() < 0.1)
				{
					...
					balloons.Add(new Balloon(screenWidth, screenHeight));
					balloons.Last().position = new Vector2(vector.X + (float)Game1.random.Next(-16, 16), vector.Y + (float)Game1.random.Next(8));
					balloons.Add(new Balloon(screenWidth, screenHeight));
					balloons.Last().position = new Vector2(vector.X + (float)Game1.random.Next(-16, 16), vector.Y + (float)Game1.random.Next(8));
					balloons.Add(new Balloon(screenWidth, screenHeight));
					balloons.Last().position = new Vector2(vector.X + (float)Game1.random.Next(-16, 16), vector.Y + (float)Game1.random.Next(8));
		...
		public bool tick(GameTime time)
		{
			...
			case 2:
			{
				int num3 = screenHeight * 3;
				int num4 = Game1.viewport.Height - 1020;
		...
		public void drawRoadArea(SpriteBatch b)
		{
			...
			for (int i = 0; i < screenWidth / 112 + 2; i++)
			{
				if (sky[i] == 0)
				{
					b.Draw(texture, new Rectangle((int)(0f - skyPosition + (float)(i * 112)), (int)(-16f * Game1.options.zoomLevel), 114, 150), new Rectangle(128, 0, 112, 96), Color.White);
				}
				else
				{
					Rectangle rectangle = new Rectangle(128, 0, 1, 96);
					b.Draw(sourceRectangle: new Rectangle(129, 0, 1, 96), texture: texture, destinationRectangle: new Rectangle((int)(0f - skyPosition) - 1 + i * 112, (int)(-16f * Game1.options.zoomLevel), 114, 150), color: Color.White);
				}
			}
			for (int j = 0; j < 8; j++)
			{
				b.Draw(Game1.mouseCursors, new Vector2(-10f + (0f - valleyPosition) / 2f + (float)(j * 639), 70f), new Rectangle(0, 886, 639, 148), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.08f);
				b.Draw(Game1.mouseCursors, new Vector2(0f - valleyPosition + (float)(j * 639), 80f), new Rectangle(0, 737, 639, 120), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.08f);
			}
			...
			for (int l = 0; l < roadsideObjects.Count; l++)
			{
				switch (roadsideObjects[l])
				{
				case 0:
					b.Draw(roadsideTexture, new Vector2((float)(l * 16) - roadPosition - 32f, 96f), new Rectangle(48, 0, 48, 96), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					break;
				case 1:
					b.Draw(roadsideTexture, new Vector2((float)(l * 16) - roadPosition - 32f, 96f), new Rectangle(0, 0, 48, 64), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					b.Draw(roadsideTexture, new Vector2((float)(l * 16) - roadPosition - 16f, 160f), new Rectangle(16, 64, 16, 32), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					break;
				case 2:
					b.Draw(roadsideTexture, new Vector2((float)(l * 16) - roadPosition - 32f, 176f), new Rectangle(112, 144, 16, 16), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					break;
				case 3:
					b.Draw(roadsideTexture, new Vector2((float)(l * 16) - roadPosition - 32f, 176f), new Rectangle(112, 160, 16, 16), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					break;
				case 5:
					b.Draw(texture, new Vector2((float)(l * 16) - roadPosition - 32f, 128f), new Rectangle(48, 176, 64, 64), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					break;
				}
			}
			for (int m = 0; m < roadsideFences.Count; m++)
			{
				if (roadsideFences[m] != -1)
				{
					if (roadsideFences[m] == 3)
					{
						b.Draw(roadsideTexture, new Vector2((float)(m * 16) - roadPosition, 176f), new Rectangle(144, 256, 16, 32), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					}
					else
					{
						b.Draw(roadsideTexture, new Vector2((float)(m * 16) - roadPosition, 176f), new Rectangle(128 + roadsideFences[m] * 16, 224, 16, 32), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					}
				}
			}
