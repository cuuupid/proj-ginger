...
namespace StardewValley.Minigames
{
	public class FishingGame : IMinigame
	{
		...
		public FishingGame()
		{
			...
			Game1.player.Halt();
			upperRightCloseButton = new ClickableTextureComponent(new Microsoft.Xna.Framework.Rectangle(Game1.uiViewport.Width - 68 - Game1.xEdge, 0, 68 + Game1.xEdge, 80), Game1.mobileSpriteSheet, new Microsoft.Xna.Framework.Rectangle(62, 0, 17, 17), 4f, drawShadow: true);
		}
		...
		public bool tick(GameTime time)
		{
			...
			{
				if (Game1.screenOverlayTempSprites[num].update(time))
				{
			...
			{
				int num2 = showResultsTimer;
				showResultsTimer -= time.ElapsedGameTime.Milliseconds;
				if (num2 > 11000 && showResultsTimer <= 11000)
				{
					Game1.playSound("smallSelect");
				}
				if (num2 > 9000 && showResultsTimer <= 9000)
				{
					Game1.playSound("smallSelect");
				}
				if (num2 > 7000 && showResultsTimer <= 7000)
				{
					if (perfections > 0)
					{
				...
				if (num2 > 5000 && showResultsTimer <= 5000)
				{
					if (score >= 10)
					{
		...
		{
			if (!exit && upperRightCloseButton.containsPoint(x, y))
			{
				ExitGame();
			}
		...
		public void ReceiveMobileKeyStates(MobileKeyStates mobileKeyStates)
		{
			...
			if (!gameDone && !Game1.player.UsingTool && timerToStart <= 0)
			{
			...
			if (_leftClickNextUpdate)
			{
				OnLeftClick();
				_leftClickNextUpdate = false;
			}
			if (mobileKeyStates.useToolButtonPressed)
			{
				_leftClickNextUpdate = true;
			}
			if (mobileKeyStates.useToolButtonReleased)
			{
				OnLeftClickRelease((int)location.tapToMove.ClickPoint.X, (int)location.tapToMove.ClickPoint.Y);
			}
			if (mobileKeyStates.actionButtonPressed)
			{
				OnRightClick();
			}
		}
		...
		private void OnLeftClick()
		{
			...
			{
				if (timerToStart <= 0 && showResultsTimer < 0 && !gameDone && Game1.activeClickableMenu == null && !(Game1.player.CurrentTool as FishingRod).hit && !(Game1.player.CurrentTool as FishingRod).pullingOutOfWater && !(Game1.player.CurrentTool as FishingRod).isCasting && !(Game1.player.CurrentTool as FishingRod).fishCaught)
				{
					Game1.player.lastClick = Vector2.Zero;
					Game1.player.Halt();
					Game1.pressUseToolButton();
		...
		{
			if (showResultsTimer < 0 && Game1.player.CurrentTool != null && !(Game1.player.CurrentTool as FishingRod).isCasting && Game1.activeClickableMenu == null && Game1.player.CurrentTool.onRelease(location, x, y, Game1.player))
			{
				Game1.player.Halt();
			}
		}
