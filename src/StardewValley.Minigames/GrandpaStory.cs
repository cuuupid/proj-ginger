...
namespace StardewValley.Minigames
{
	public class GrandpaStory : IMinigame
	{
		...
		public GrandpaStory()
		{
			PinchZoom.Instance.SetZoomLevel(Game1.NativeZoomLevel);
			Game1.options.desiredBaseZoomLevel = (Game1.options.baseZoomLevel = Game1.NativeZoomLevel);
		...
		public bool tick(GameTime time)
		{
			...
			case 1:
			{
				...
				height += 100;
				if (letterReceived && letterPosition.Y <= (float)height)
				{
					...
					if (letterPosition.Y > (float)height)
					{
		...
		public void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (letterView != null)
			{
				x = Game1.getMouseX(ui_scale: true);
				y = Game1.getMouseY(ui_scale: true);
		...
		public void draw(SpriteBatch b)
		{
			...
			Vector2 topLeftPositionForCenteringOnScreen = Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 1294, 730);
			if (drawGrandpa)
			{
				b.Draw(texture, topLeftPositionForCenteringOnScreen, new Rectangle(427, (totalMilliseconds % 300 < 150) ? 240 : 0, 427, 240), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(317f, 74f) * 3f, new Rectangle(427 + 74 * (totalMilliseconds % 400 / 100), 480, 74, 42), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(320f, 75f) * 3f, new Rectangle(427, 522, 70, 32), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				if (grandpaSpeechTimer > 8000 && grandpaSpeechTimer % 10000 < 5000)
				{
					b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(189f, 69f) * 3f, new Rectangle(497 + 18 * (totalMilliseconds % 400 / 200), 523, 18, 18), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				}
				if (grandpaSpeech.Count > 0 && grandpaSpeechTimer > 3000)
				{
					float num = 1f;
					if (Game1.viewport.Width <= 1280)
					{
						num = 0.75f;
					}
					...
					vector *= num;
					float num2 = 3f * num;
					Vector2 position = new Vector2((float)(Game1.viewport.Width / 2) - vector.X / 2f, (float)((int)topLeftPositionForCenteringOnScreen.Y + 669) + 3f);
					int num3 = Game1.viewport.Height - ((int)topLeftPositionForCenteringOnScreen.Y + 730);
					int num4 = 20;
					position.Y = (float)((int)topLeftPositionForCenteringOnScreen.Y + 730) + ((float)num3 - vector.Y) / 2f;
					if (position.Y + vector.Y > (float)Game1.viewport.Height)
					{
						position.Y = (float)Game1.viewport.Height - vector.Y - (float)num4;
					}
					b.Draw(Game1.staminaRect, new Rectangle(0, (int)position.Y - num4, Game1.viewport.Width, (int)vector.Y + 3 * num4), Color.Black);
					position.X -= num2;
				...
				if (letterReceived)
				{
					b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(157f, 113f) * 3f, new Rectangle(463, 556, 37, 17), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
					if (grandpaSpeechTimer > 8000 && grandpaSpeechTimer % 10000 > 7000 && grandpaSpeechTimer % 10000 < 9000 && totalMilliseconds % 600 < 300)
					{
						b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(157f, 113f) * 3f, new Rectangle(500, 556, 37, 17), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
					}
					b.Draw(texture, topLeftPositionForCenteringOnScreen + letterPosition, new Rectangle(729, 524, 131, 63), Color.White, 0f, Vector2.Zero, letterScale, SpriteEffects.None, 1f);
				}
			}
			else if (scene == 3)
			{
			...
			{
				float num5 = 1f - ((float)grandpaSpeechTimer - 7000f) / 2000f;
				b.Draw(texture, topLeftPositionForCenteringOnScreen, new Rectangle(0, 0, 427, 240), Color.White * num5, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(22f, 211f) * 3f, new Rectangle(264 + totalMilliseconds % 500 / 250 * 19, 581, 19, 17), Color.White * num5, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(332f, 215f) * 3f, new Rectangle(305 + totalMilliseconds % 600 / 200 * 12, 581, 12, 12), Color.White * num5, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(414f, 211f) * 3f, new Rectangle(460 + totalMilliseconds % 400 / 200 * 13, 581, 13, 17), Color.White * num5, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(189f, 81f) * 3f, new Rectangle(426 + totalMilliseconds % 800 / 400 * 16, 581, 16, 16), Color.White * num5, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
			}
			...
			if (scene == 6)
			{
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(261 - parallaxPan, 145f) * 4f, new Rectangle(550, 540, 56 + parallaxPan, 35), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(261 - parallaxPan, 4f + (float)Math.Max(0, Math.Min(60, (grandpaSpeechTimer - 5000) / 8))) * 4f, new Rectangle(264, 434, 56 + parallaxPan, 141), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
				if (grandpaSpeechTimer > 3000)
				{
					b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(286 - parallaxPan, 32f + (float)Math.Max(0, Math.Min(60, (grandpaSpeechTimer - 5000) / 8)) + Math.Min(30f, (float)letterOpenTimer / 4f)) * 4f, new Rectangle(729 + Math.Min(2, letterOpenTimer / 200) * 131, 508, 131, 79), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
				}
				b.Draw(texture, topLeftPositionForCenteringOnScreen, new Rectangle(parallaxPan, 240, 320, 180), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
				b.Draw(texture, topLeftPositionForCenteringOnScreen + new Vector2(187f - (float)parallaxPan * 2.5f, 8f) * 4f, new Rectangle(20, 428, 232, 172), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
			}
			b.End();
			Game1.PushUIMode();
			b.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp);
