...
namespace StardewValley.Tools
{
	public class MilkPail : Tool
	{
		...
		public override Item getOne()
		{
			...
			CopyEnchantments(this, milkPail);
			milkPail._GetOneFrom(this);
			return milkPail;
		}
		...
		public override bool beginUsing(GameLocation location, int x, int y, Farmer who)
		{
			...
			if (location is Farm)
			{
				animal = Utility.GetBestHarvestableFarmAnimal((location as Farm).animals.Values, this, toolRect);
			}
			else if (location is AnimalHouse)
			{
				animal = Utility.GetBestHarvestableFarmAnimal((location as AnimalHouse).animals.Values, this, toolRect);
			}
			...
			else if (who != null && Game1.player.Equals(who))
			{
				...
				{
					text = Game1.content.LoadString("Strings\\StringsFromCSFiles:MilkPail.cs.14175", animal.displayName);
				}
				if (animal != null && animal.isBaby() && animal.toolUsedForHarvest.Equals(base.BaseName))
				{
					text = Game1.content.LoadString("Strings\\StringsFromCSFiles:MilkPail.cs.14176", animal.displayName);
				}
				if (animal != null && (int)animal.age >= (byte)animal.ageWhenMature && animal.toolUsedForHarvest.Equals(base.BaseName))
				{
					text = Game1.content.LoadString("Strings\\StringsFromCSFiles:MilkPail.cs.14177", animal.displayName);
				}
				if (text.Length > 0)
				{
