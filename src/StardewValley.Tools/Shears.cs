	...
	public class Shears : Tool
	{
		...
		public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
		{
			...
			else
			{
				...
				{
					text = Game1.content.LoadString("Strings\\StringsFromCSFiles:Shears.cs.14245", animal.displayName);
				}
				if (animal != null && animal.isBaby() && animal.toolUsedForHarvest.Equals(base.BaseName))
				{
					text = Game1.content.LoadString("Strings\\StringsFromCSFiles:Shears.cs.14246", animal.displayName);
				}
				if (animal != null && (int)animal.age >= (byte)animal.ageWhenMature && animal.toolUsedForHarvest.Equals(base.BaseName))
				{
					text = Game1.content.LoadString("Strings\\StringsFromCSFiles:Shears.cs.14247", animal.displayName);
				}
				if (text.Length > 0)
				{
