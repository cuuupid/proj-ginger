		...
		public struct PairsCollection : IEnumerable<KeyValuePair<Vector2, Object>>, IEnumerable
		{
			public struct Enumerator : IEnumerator<KeyValuePair<Vector2, Object>>, IDisposable, IEnumerator
			{
				...
				public bool MoveNext()
				{
					...
					{
						KeyValuePair<Vector2, Object> current = _base.Current;
						if (!_dict.overlayDict.ContainsKey(current.Key))
						{
							_current = new KeyValuePair<Vector2, Object>(current.Key, current.Value);
							return true;
						}
					}
					if (_overlay.MoveNext())
					{
						KeyValuePair<Vector2, Object> current2 = _overlay.Current;
						_current = new KeyValuePair<Vector2, Object>(current2.Key, current2.Value);
						return true;
					}
					_done = true;
					_current = default(KeyValuePair<Vector2, Object>);
