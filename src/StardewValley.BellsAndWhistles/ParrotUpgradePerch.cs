	...
	public class ParrotUpgradePerch : INetObject<NetFields>
	{
		...
		public class Parrot
		{
			...
			public virtual void FindNewTarget()
			{
				...
				moveTime = 0f;
				moveDuration = 1f;
				Microsoft.Xna.Framework.Rectangle r = _perch.upgradeRect;
				...
				targetPosition = (Utility.getRandomPositionInThisRectangle(r, Game1.random) + new Vector2(0.5f, 0.5f)) * 64f;
		...
		public virtual void PerformCompleteAnimation()
		{
			if (upgradeName.Contains("Volcano"))
			{
				for (int i = 0; i < 16; i++)
				{
					...
					TemporaryAnimatedSprite temporaryAnimatedSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(65, 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect, Game1.random) * 64f, Game1.random.NextDouble() < 0.5, 0f, Color.White)
					{
					...
					temporaryAnimatedSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(65, 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect, Game1.random) * 64f, Game1.random.NextDouble() < 0.5, 0f, Color.White)
					{
						motion = new Vector2(Game1.random.Next(-2, 3), -16f),
						acceleration = new Vector2(0f, 0.5f),
						rotationChange = (float)Game1.random.Next(-4, 5) * 0.05f,
			...
			else if (upgradeName == "House")
			{
				for (int j = 0; j < 16; j++)
				{
					...
					TemporaryAnimatedSprite temporaryAnimatedSprite2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(49 + 16 * Game1.random.Next(3), 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect, Game1.random) * 64f, Game1.random.NextDouble() < 0.5, 0f, Color.White)
					{
						motion = new Vector2(Game1.random.Next(-2, 3), -16f),
						acceleration = new Vector2(0f, 0.5f),
						rotationChange = (float)Game1.random.Next(-4, 5) * 0.05f,
					...
					temporaryAnimatedSprite2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(49 + 16 * Game1.random.Next(3), 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect, Game1.random) * 64f, Game1.random.NextDouble() < 0.5, 0f, Color.White)
					{
						motion = new Vector2(Game1.random.Next(-2, 3), -16f),
						acceleration = new Vector2(0f, 0.5f),
						rotationChange = (float)Game1.random.Next(-4, 5) * 0.05f,
			...
			else if ((upgradeName == "Resort" || upgradeName == "Trader" || upgradeName == "Obelisk") && locationRef.Value == Game1.currentLocation)
			{
				Game1.flashAlpha = 1f;
			}
		...
		public bool CheckAction(Location tile_location, Farmer farmer)
		{
			...
			{
				string text = Game1.content.LoadStringReturnNullIfNotFound("Strings\\UI:UpgradePerch_" + upgradeName.Value);
		...
		public virtual void Update(GameTime time)
		{
			...
			if (parrotPresent && currentState.Value > UpgradeState.StartBuilding)
			{
				...
				{
					Parrot parrot = new Parrot(this, Utility.PointToVector2(tilePosition));
			...
			for (int j = 0; j < parrots.Count; j++)
			{
				Parrot parrot2 = parrots[j];
				if (parrot2.Update(time))
				{
					parrots.RemoveAt(j);
					j--;
				}
		...
		public virtual void Draw(SpriteBatch b)
		{
			if (IsAvailable(use_cached_value: true) && (parrotPresent || upgradeName == "Hut"))
			{
				...
				b.Draw(texture, Game1.GlobalToLocal(Game1.viewport, (Utility.PointToVector2(tilePosition) + new Vector2(0.5f, -1f)) * 64f) + zero, new Microsoft.Xna.Framework.Rectangle(num * 24, 0, 24, 24), Color.White, 0f, new Vector2(12f, 16f), 4f, SpriteEffects.None, (((float)tilePosition.Y + 1f) * 64f - 1f) / 10000f);
			}
		}
