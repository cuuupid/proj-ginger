		...
		public void UpdateGlobalFade()
		{
			if (fadeIn)
			{
				if (fadeToBlackAlpha <= 0f)
				{
					...
					if (afterFade != null)
					{
						Game1.afterFadeFunction obj = afterFade;
						afterFade();
						if (afterFade != null && afterFade.Equals(obj))
						{
							afterFade = null;
						}
			...
			if (fadeToBlackAlpha >= 1f)
			{
				...
				if (afterFade != null)
				{
					Game1.afterFadeFunction obj2 = afterFade;
					afterFade();
					if (afterFade != null && afterFade.Equals(obj2))
					{
						afterFade = null;
					}
					if (Game1.nonWarpFade)
