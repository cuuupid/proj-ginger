	...
	public class MoneyDial
	{
		...
		public void draw(SpriteBatch b, Vector2 position, int target)
		{
			...
			for (int i = 0; i < numDigits; i++)
			{
				...
				{
					float num5 = 1f;
					b.Draw(Game1.mouseCursors, position + new Vector2(num2, (Game1.activeClickableMenu != null && Game1.activeClickableMenu is ShippingMenu && currentValue >= 1000000) ? ((float)Math.Sin(Game1.currentGameTime.TotalGameTime.TotalMilliseconds / 100.53096771240234 + (double)i) * (float)(currentValue / 1000000)) : 0f), new Rectangle(286, 502 - num4 * 8, 5, 8), Color.Maroon, 0f, Vector2.Zero, num5 * (4f + ((moneyShineTimer / 60 == numDigits - i) ? 0.3f : 0f)), SpriteEffects.None, 2E-07f);
				}
				num2 += 24;
				num3 /= 10;
			}
