	...
	public class Squirrel : Critter
	{
		...
		public override bool update(GameTime time, GameLocation environment)
		{
			...
			if (sprite.CurrentAnimation == null && nextNibbleTimer <= 0)
			{
				...
				for (int i = 0; i < num; i++)
				{
					list.Add(new FarmerSprite.AnimationFrame(baseFrame, 200));
					list.Add(new FarmerSprite.AnimationFrame(baseFrame + 1, 200));
				}
				list.Add(new FarmerSprite.AnimationFrame(baseFrame, 200, secondaryArm: false, flip: false, doneNibbling));
				sprite.setCurrentAnimation(list);
			}
			characterCheckTimer -= time.ElapsedGameTime.Milliseconds;
			if (characterCheckTimer <= 0 && !running)
			{
				Character character = Utility.isThereAFarmerOrCharacterWithinDistance(position / 64f, 12, environment);
				if (character != null)
				{
					running = true;
			...
			if (running && characterCheckTimer <= 0 && treeRunTimer <= 0)
			{
				...
				Vector2 vector = new Vector2((int)(position.X / 64f), (int)position.Y / 64);
				if (environment.terrainFeatures.ContainsKey(vector) && environment.terrainFeatures[vector] is Tree)
				{
					treeRunTimer = 700;
					climbed = environment.terrainFeatures[vector] as Tree;
					treeTile = vector;
					position = vector * 64f;
					return false;
				}
				vector = new Vector2((int)((position.X + 64f + 1f) / 64f), (int)position.Y / 64);
			}
			if (treeRunTimer > 0)
			{
