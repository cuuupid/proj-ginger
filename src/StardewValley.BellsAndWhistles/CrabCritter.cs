...
namespace StardewValley.BellsAndWhistles
{
	public class CrabCritter : Critter
	{
			...
			: this()
		{
			...
			float num = 256f;
			movementBounds = new Microsoft.Xna.Framework.Rectangle((int)(start_position.X - num / 2f), (int)start_position.Y, (int)num, 0);
		}
		...
		public override bool update(GameTime time, GameLocation environment)
		{
			...
			else
			{
				...
				{
					Character character = Utility.isThereAFarmerOrCharacterWithinDistance(position / 64f, 7, environment);
					if (character != null)
					{
						_crabVariant = 0;
						skittering = true;
		...
		{
			Microsoft.Xna.Framework.Rectangle baseSourceRectangle = _baseSourceRectangle;
			baseSourceRectangle.Y += _crabVariant * 18;
			int num = _currentFrame;
			if (num == 3)
			{
				num = 1;
			}
			baseSourceRectangle.X += num * 18;
			sprite.SourceRect = baseSourceRectangle;
		}

		public override void draw(SpriteBatch b)
		{
			float num = skitterTime;
			if (num > 1f)
			{
				num = 1f;
			}
			...
			sprite.draw(b, Game1.GlobalToLocal(Game1.viewport, Utility.snapDrawPosition(position + new Vector2(0f, -20f + yJumpOffset + yOffset))), (position.Y + 64f - 32f) / 10000f, 0, 0, Color.White * num, flip, 4f);
			b.Draw(Game1.shadowTexture, Game1.GlobalToLocal(Game1.viewport, position + new Vector2(32f, 40f)), Game1.shadowTexture.Bounds, Color.White * num, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 3f + Math.Max(-3f, (yJumpOffset + yOffset) / 16f), SpriteEffects.None, (position.Y - 1f) / 10000f);
		}
