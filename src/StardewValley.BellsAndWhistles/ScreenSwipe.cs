...
namespace StardewValley.BellsAndWhistles
{
	public class ScreenSwipe
	{
		...
		private int ViewportWidth => width;

		private int ViewportHeight => height;

		public ScreenSwipe(int which, float swipeVelocity, int durationAfterSwipe, int w, int h)
		{
			...
			width = w;
			height = h;
			Vector2 vector = new Vector2(ViewportWidth / 2, ViewportHeight / 2);
			if (which == 0)
			{
			...
			if (which == 0)
			{
				...
				yPosition = (int)vector.Y - bgSource.Height * 4 / 2;
				messagePosition = new Vector2(vector.X - (float)(messageSource.Width * 4 / 2), vector.Y - (float)(messageSource.Height * 4 / 2));
				...
				movingFlairPosition = new Vector2(messagePosition.X + (float)(messageSource.Width * 4) + 192f, vector.Y + 32f);
		...
		public Rectangle getAdjustedSourceRect(Rectangle sourceRect, float xStartPosition)
		{
			...
			int num = (int)Math.Max(sourceRect.X, (float)sourceRect.X + ((float)bgDest.X - xStartPosition) / 4f);
			return new Rectangle(num, sourceRect.Y, (int)Math.Min(sourceRect.Width - (num - sourceRect.X) / 4, ((float)bgDest.Width - xStartPosition) / 4f), sourceRect.Height);
		}

		public void draw(SpriteBatch b)
		{
			b.Draw(Game1.mouseCursors, bgDest, bgSource, Color.White);
			foreach (Vector2 flairPosition in flairPositions)
			{
				Rectangle adjustedSourceRect = getAdjustedSourceRect(flairSource, flairPosition.X);
				if (adjustedSourceRect.Right >= originalBGSourceXLimit)
				{
					adjustedSourceRect.Width = originalBGSourceXLimit - adjustedSourceRect.X;
				}
				b.Draw(Game1.mouseCursors, flairPosition, adjustedSourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
			}
