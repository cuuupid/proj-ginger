...
namespace StardewValley.BellsAndWhistles
{
	public class CalderaMonkey : Critter
	{
		...
		{
			sprite.draw(b, Game1.GlobalToLocal(Game1.viewport, Utility.snapDrawPosition(position + new Vector2(0f, -20f + yJumpOffset + yOffset))), (position.Y + 64f - 32f) / 10000f, 0, 0, Color.White, flip, 4f);
			for (int i = 0; i < buddies.Count; i++)
			{
				float num = (float)Math.Sin((double)(float)Game1.currentGameTime.TotalGameTime.TotalMilliseconds / 500.0 + (double)(i * 100)) * 4f;
				Vector2 vector = Game1.GlobalToLocal(Game1.viewport, buddies[i]);
				b.Draw(texture, vector + new Vector2(0f, num), new Rectangle(14 * i, 333, 14, 12 - (int)num / 2), Color.White, 0f, Vector2.Zero, 4f, (vector.X > 1408f) ? SpriteEffects.FlipHorizontally : SpriteEffects.None, buddies[i].Y / 10000f);
				b.Draw(Game1.staminaRect, new Rectangle((int)vector.X + (int)num + 8, (int)vector.Y + 44 + 2, 56 - (int)num * 2 - 16, 4), Game1.staminaRect.Bounds, new Color(255, 255, 150) * 0.55f, 0f, Vector2.Zero, SpriteEffects.None, buddies[i].Y / 10000f + 0.001f);
				b.Draw(swimShadow, vector + new Vector2(-4f, 48f), new Rectangle((int)Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 700 / 70 * 16, 0, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, buddies[i].Y / 10000f - 0.001f);
			}
		}
