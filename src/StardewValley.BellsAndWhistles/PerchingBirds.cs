...
namespace StardewValley.BellsAndWhistles
{
	public class PerchingBirds
	{
		...
		public Point GetFreeBirdPoint(Bird bird = null, int clearance = 200)
		{
			...
			for (int i = 0; i < currentBirdLocationList.Length; i++)
			{
				...
				if (bird != null)
				{
					...
					{
						float num = Utility.distance(farmer.position.X, (float)(point.X * 64) + 32f, farmer.position.Y, (float)(point.Y * 64) + 32f);
						if (num < 200f)
						{
							flag = true;
						}
		...
		public virtual void ResetLocalState()
		{
			...
			Point[] birdRoostLocations = _birdRoostLocations;
			foreach (Point key2 in birdRoostLocations)
			{
