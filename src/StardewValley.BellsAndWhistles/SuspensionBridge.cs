	...
	public class SuspensionBridge
	{
		...
		public virtual void Update(GameTime time)
		{
			...
			if (Game1.player.bridge == this)
			{
				...
				if (boundingBox.Top >= bridgeBounds.Top && boundingBox.Bottom <= bridgeBounds.Bottom && (boundingBox.Intersects(bridgeBounds) || InEntranceArea(boundingBox)))
				{
					Game1.player.SetOnBridge(val: true);
				}
				else if (!InEntranceArea(Game1.player.GetBoundingBox()) && !boundingBox.Intersects(bridgeBounds))
				{
		...
		public virtual void Draw(SpriteBatch b)
		{
			...
			float[] array = new float[6] { 0f, 0.5f, 1f, 1f, 0.5f, 0f };
			for (int i = 0; i < 6; i++)
			{
				float num = (float)Math.Sin(Game1.currentGameTime.TotalGameTime.TotalSeconds * 10.0 + (double)(i * 5)) * 1f * 4f * array[i] * (shakeTime / 0.4f);
				b.Draw(_texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(bridgeBounds.X + i * 64, (float)bridgeBounds.Y + num)), new Rectangle(16 * i, 32, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)bridgeBounds.Y / 10000f + 0.0256f);
			}
		}
	}
}
