	...
	public class TrainCar : INetObject<NetFields>
	{
		...
		public void draw(SpriteBatch b, Vector2 globalPosition, float wheelRotation)
		{
			...
			if ((int)carType == 1)
			{
				...
				if ((int)loaded > 0 && Game1.random.NextDouble() < 0.02 && globalPosition.X > 320f && globalPosition.X < (float)(Game1.currentLocation.map.DisplayWidth - 256))
				{
					...
					case 0:
						...
						break;
					case 1:
						num = ((color.R > color.G) ? 378 : ((color.G > color.B) ? 380 : ((color.B > color.R) ? 384 : 378)));
						break;
					case 7:
						num = (Game1.currentSeason.Equals("winter") ? 536 : ((Game1.stats.DaysPlayed > 120 && color.R > color.G) ? 537 : 535));
						break;
					case 2:
						num = 388;
						break;
					...
					if (num != -1)
					{
						Game1.createObjectDebris(num, (int)globalPosition.X / 64 + 2, (int)globalPosition.Y / 64, (int)(globalPosition.Y + 320f));
					}
			...
			if ((int)carType == 3)
			{
				...
				int num2 = (int)((double)(position.X + 4f) + 24.0 * Math.Cos(wheelRotation));
				int num3 = (int)((double)(position.Y + 4f) + 24.0 * Math.Sin(wheelRotation));
				int num4 = (int)((double)(position2.X + 4f) + 24.0 * Math.Cos(wheelRotation));
				int num5 = (int)((double)(position2.Y + 4f) + 24.0 * Math.Sin(wheelRotation));
				...
				b.Draw(Game1.mouseCursors, new Vector2(num2 - 8, num3 - 8), new Rectangle(192, 640, 24, 24), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 268f) / 10000f);
				b.Draw(Game1.mouseCursors, new Vector2(num4 - 8, num5 - 8), new Rectangle(192, 640, 24, 24), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 268f) / 10000f);
			}
		}
	}
}
