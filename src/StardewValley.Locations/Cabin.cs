...
namespace StardewValley.Locations
{
	...
	{
		internal static Random farmhandIDRandom = new Random();
		...
		public NetRef<Farmer> getFarmhand()
		{
			if (farmhand.Value == null)
			{
				...
				SocializeQuest item = Quest.getQuestFromId(9) as SocializeQuest;
				farmhand.Value.questLog.Add(item);
				resetFarmhandState();
			}
			return farmhand;
		}
		...
		public void resetFarmhandState()
		{
			if (farmhand.Value != null)
			{
				...
				if (farmhand.Value.lastSleepLocation.Value == null || farmhand.Value.lastSleepLocation.Value == uniqueName)
				{
		...
		private void grabItemFromPlayerInventory(Item item, Farmer who)
		{
			...
			Item item2 = getFarmhand().Value.addItemToInventory(item);
			if (item2 == null)
			{
		...
		public List<Item> demolish()
		{
			...
			{
				if (item.isVillager() && dictionary.ContainsKey(item.Name))
				{
				...
				if (item is Pet)
				{
					(item as Pet).warpToFarmHouse(Game1.MasterPlayer);
				}
			}
			if (Game1.getLocationFromName(GetCellarName()) is Cellar cellar)
			{
