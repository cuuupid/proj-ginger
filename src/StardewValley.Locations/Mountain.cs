	...
	public class Mountain : GameLocation
	{
			...
			if (map.GetLayer("Buildings").Tiles[tileLocation] != null)
			{
				...
				case 1081:
					...
					if (Game1.MasterPlayer.mailReceived.Contains("ccBoilerRoom"))
					{
						...
						createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:MineCart_ChooseDestination"), answerChoices, "Minecart");
						break;
					}
		...
		private void restoreBridge()
		{
			...
			for (int i = 0; i < map.GetLayer("Back").LayerWidth; i++)
			{
				...
				{
					base.map.GetLayer("Back").Tiles[i + num, j + num2] = ((map.GetLayer("Back").Tiles[i, j] == null) ? null : new StaticTile(base.map.GetLayer("Back"), base.map.TileSheets[0], BlendMode.Alpha, map.GetLayer("Back").Tiles[i, j].TileIndex));
					base.map.GetLayer("Buildings").Tiles[i + num, j + num2] = ((map.GetLayer("Buildings").Tiles[i, j] == null) ? null : new StaticTile(base.map.GetLayer("Buildings"), base.map.TileSheets[0], BlendMode.Alpha, map.GetLayer("Buildings").Tiles[i, j].TileIndex));
					base.map.GetLayer("Front").Tiles[i + num, j + num2] = ((map.GetLayer("Front").Tiles[i, j] == null) ? null : new StaticTile(base.map.GetLayer("Front"), base.map.TileSheets[0], BlendMode.Alpha, map.GetLayer("Front").Tiles[i, j].TileIndex));
				}
			}
			bridgeRestored = true;
			localizedContentManager.Unload();
			localizedContentManager = null;
		}
		...
		protected override void resetSharedState()
		{
			...
			if (!objects.ContainsKey(new Vector2(29f, 9f)))
			{
				...
				objects.Add(vector, new Torch(vector, 146, bigCraftable: true)
				{
					IsOn = false,
					Fragility = 2
				});
				objects[vector].checkForAction(null);
			}
		...
		private void quarryDayUpdate()
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (!isTileOpenForQuarryStone((int)randomPositionInThisRectangle.X, (int)randomPositionInThisRectangle.Y))
				{
					continue;
				}
				if (Game1.random.NextDouble() < 0.06)
				{
					terrainFeatures.Add(randomPositionInThisRectangle, new Tree(1 + Game1.random.Next(2), 1));
				}
				else if (Game1.random.NextDouble() < 0.02)
				{
					if (Game1.random.NextDouble() < 0.1)
					{
						objects.Add(randomPositionInThisRectangle, new Object(randomPositionInThisRectangle, 46, "Stone", canBeSetDown: true, canBeGrabbed: false, isHoedirt: false, isSpawnedObject: false)
						{
					...
					{
						objects.Add(randomPositionInThisRectangle, new Object(randomPositionInThisRectangle, (Game1.random.Next(7) + 1) * 2, "Stone", canBeSetDown: true, canBeGrabbed: false, isHoedirt: false, isSpawnedObject: false)
						{
							MinutesUntilReady = 5
						});
					}
				...
				else if (Game1.random.NextDouble() < 0.1)
				{
					...
					{
						objects.Add(randomPositionInThisRectangle, new Object(randomPositionInThisRectangle, 765, 1)
						{
					...
					{
						objects.Add(randomPositionInThisRectangle, new Object(randomPositionInThisRectangle, 764, 1)
						{
							MinutesUntilReady = 8
						});
					}
					...
					{
						objects.Add(randomPositionInThisRectangle, new Object(randomPositionInThisRectangle, 290, 1)
						{
							MinutesUntilReady = 5
						});
					}
					...
					{
						objects.Add(randomPositionInThisRectangle, new Object(randomPositionInThisRectangle, 751, 1)
						{
							MinutesUntilReady = 3
						});
					}
		...
		public override Object getFish(float millisecondsAfterNibble, int bait, int waterDepth, Farmer who, double baitPotency, Vector2 bobberTile, string locationName = null)
		{
			...
			if ((Game1.player.team.SpecialOrderRuleActive("LEGENDARY_FAMILY") || Game1.isRaining || flag) && who.FishingLevel >= 10 && waterDepth >= 4 && Game1.random.NextDouble() < 0.1 + (double)num)
			{
		...
		public override bool isTilePlaceable(Vector2 tile_location, Item item = null)
		{
			...
			if ((bool)landslide && landSlideRect.Contains(value))
			{
				return false;
			}
			if ((bool)railroadAreaBlocked && railroadBlockRect.Contains(value))
			{
				return false;
			}
