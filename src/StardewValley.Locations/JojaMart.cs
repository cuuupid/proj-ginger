...
namespace StardewValley.Locations
{
	public class JojaMart : GameLocation
	{
		...
		public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
		{
			...
			{
				PropertyValue value = "";
				map.GetLayer("Buildings").Tiles[tileLocation].Properties.TryGetValue("Action", out value);
				if (value != null)
				{
					string text = value.ToString();
					if (!(text == "JojaShop") && text == "JoinJoja")
					{
						...
						else if (!Game1.player.mailReceived.Contains("JojaMember"))
						{
							...
							else if (Game1.stats.DaysPlayed < 0)
							{
								...
								Morris.setNewDialogue(Game1.content.LoadString(path));
								Game1.drawDialogue(Morris);
							}
							else
							{
								string text2 = ((Game1.dayOfMonth % 7 == 0 || Game1.dayOfMonth % 7 == 6) ? "Data\\ExtraDialogue:Morris_WeekendGreeting" : "Data\\ExtraDialogue:Morris_FirstGreeting");
								if (Game1.IsMasterGame)
								{
									if (!Game1.player.eventsSeen.Contains(611439))
									{
										Morris.setNewDialogue(Game1.content.LoadString(text2));
										Game1.drawDialogue(Morris);
									}
									else if (Game1.player.mailReceived.Contains("ccIsComplete"))
									{
										Morris.setNewDialogue(Game1.content.LoadString(text2 + "_CommunityCenterComplete"));
										Game1.drawDialogue(Morris);
									}
									else
									{
										Morris.setNewDialogue(Game1.content.LoadString(text2 + "_MembershipAvailable", 5000));
								...
								{
									Morris.setNewDialogue(Game1.content.LoadString(text2 + "_SecondPlayer"));
									Game1.drawDialogue(Morris);
								}
							}
						}
