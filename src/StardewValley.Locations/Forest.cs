...
namespace StardewValley.Locations
{
	public class Forest : GameLocation
	{
		...
		private void showCommunityUpgradeShortcuts()
		{
			...
			{
				if (largeTerrainFeature2.tilePosition.Equals(new Vector2(119f, 35f)))
				{
		...
		protected virtual bool isWizardHouseUnlocked()
		{
			...
			bool flag2 = Game1.MasterPlayer.mailReceived.Contains("ccBulletin");
			bool flag3 = Game1.MasterPlayer.mailReceived.Contains("ccPantry");
			bool flag4 = Game1.MasterPlayer.mailReceived.Contains("ccVault");
			bool flag5 = Game1.MasterPlayer.mailReceived.Contains("ccBoilerRoom");
			bool flag6 = Game1.MasterPlayer.mailReceived.Contains("ccCraftsRoom");
			return flag && flag2 && flag3 && flag4 && flag5 && flag6;
		}

		public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
		{
			int num = ((map.GetLayer("Buildings").Tiles[tileLocation] != null) ? map.GetLayer("Buildings").Tiles[tileLocation].TileIndex : (-1));
			if (num == 901 && !isWizardHouseUnlocked())
			{
			...
			if (log != null && log.getBoundingBox(log.tile).Intersects(value))
			{
		...
		public override void DayUpdate(int dayOfMonth)
		{
			...
			if (Game1.currentSeason.Equals("spring"))
			{
				for (int i = 0; i < 7; i++)
				{
					...
					List<Vector2> list = Utility.recursiveFindOpenTiles(this, vector, 16);
					foreach (Vector2 item in list)
					{
						...
						{
							terrainFeatures.Add(item, new HoeDirt(0, new Crop(forageCrop: true, 1, (int)item.X, (int)item.Y)));
						}
					}
				}
			}
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			if (chimneyTimer <= 0)
			{
				...
				Vector2 position = (travelingMerchantDay ? new Vector2(1868f, 524f) : new Vector2(5592f, 608f));
		...
		public override void draw(SpriteBatch spriteBatch)
		{
			...
			if (travelingMerchantDay)
			{
				...
				if (Game1.timeOfDay >= 2000)
				{
					spriteBatch.Draw(Game1.staminaRect, Game1.GlobalToLocal(Game1.viewport, new Microsoft.Xna.Framework.Rectangle(1744, 640, 64, 64)), Game1.staminaRect.Bounds, Color.Black, 0f, Vector2.Zero, SpriteEffects.None, 0.076840006f);
				}
			}
