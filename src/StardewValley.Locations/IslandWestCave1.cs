	...
	public class IslandWestCave1 : IslandLocation
	{
		...
		public override bool performAction(string action, Farmer who, Location tileLocation)
		{
			if (action != null && who.IsLocalPlayer)
			{
				...
				if (array[0] == "Crystal" && (netPhase.Value == 5 || netPhase.Value == 3 || netPhase.Value == 2))
				{
					int arg = Convert.ToInt32(array[1]);
					enterValueEvent.Fire(arg);
					return true;
				}
				if (array[0] == "CrystalCaveActivate" && !isActivated && !completed.Value)
				{
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			int num2 = currentCrystalSequence[currentPlaybackCrystalSequenceIndex];
			if (num2 < crystals.Count)
			{
				crystals[num2].activate();
			}
			currentPlaybackCrystalSequenceIndex++;
			int num3 = currentDifficulty;
			...
			betweenNotesTimer = 1500f / (float)num3;
			if ((int)currentDifficulty > (((int)timesFailed < 8) ? 7 : 6))
			{
		...
		public override void TransferDataFromSavedLocation(GameLocation l)
		{
			...
			{
				IslandWestCave1 islandWestCave = l as IslandWestCave1;
