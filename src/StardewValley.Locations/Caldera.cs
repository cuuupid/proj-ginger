...
namespace StardewValley.Locations
{
	public class Caldera : IslandLocation
	{
		...
		public override Object getFish(float millisecondsAfterNibble, int bait, int waterDepth, Farmer who, double baitPotency, Vector2 bobberTile, string locationName = null)
		{
			...
			int parentSheetIndex = Game1.random.Next(167, 173);
		...
		public override bool performToolAction(Tool t, int tileX, int tileY)
		{
			if (t is WateringCan && isTileOnMap(new Vector2(tileX, tileY)) && waterTiles[tileX, tileY])
			{
				for (int i = 0; i < 10; i++)
				{
					TemporaryAnimatedSprite item = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(276, 1965, 8, 8), new Vector2((float)tileX + 0.5f, (float)tileY + 0.5f) * 64f + new Vector2(Game1.random.Next(-16, 16), Game1.random.Next(-16, 16)), flipped: false, 0.02f, Color.White)
					{
						scale = 3f,
						animationLength = 7,
