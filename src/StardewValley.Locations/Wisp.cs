...
namespace StardewValley.Locations
{
	public class Wisp
	{
		...
		public virtual void Draw(SpriteBatch b)
		{
			...
			color *= Utility.Lerp(0f, 1f, (float)Math.Sin((double)(age / lifeTime) * Math.PI));
			float num = age * rotationSpeed * 2f + rotationOffset * (float)index;
			b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, drawPosition), new Rectangle(346 + (int)(age / 0.25f + rotationOffset) % 4 * 5, 1971, 5, 5), color, num, new Vector2(2.5f, 2.5f), 4f, SpriteEffects.None, 1f);
			int num2 = oldPositionIndex;
			for (int i = 0; i < oldPositions.Length; i++)
			{
				num2++;
				if (num2 >= oldPositions.Length)
				{
					num2 = 0;
				}
				b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, oldPositions[num2]), new Rectangle(356, 1971, 5, 5), color * ((float)i / (float)oldPositions.Length), num - (float)i, new Vector2(2.5f, 2.5f), 2f, SpriteEffects.None, 1f);
			}
		}
	}
}
