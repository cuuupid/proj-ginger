...
namespace StardewValley.Projectiles
{
	public abstract class Projectile : INetObject<NetFields>
	{
		...
		private bool behaviorOnCollision(GameLocation location)
		{
			...
			foreach (Vector2 item in Utility.getListOfTileLocationsForBordersOfNonTileRectangle(getBoundingBox()))
			{
				foreach (Farmer farmer in location.farmers)
				{
					if (!damagesMonsters && farmer.GetBoundingBox().Intersects(getBoundingBox()))
					{
						behaviorOnCollisionWithPlayer(location, farmer);
						return true;
					}
				}
				if (location.terrainFeatures.ContainsKey(item) && !location.terrainFeatures[item].isPassable())
				{
					behaviorOnCollisionWithTerrainFeature(location.terrainFeatures[item], item, location);
					return true;
				}
				if ((bool)damagesMonsters)
				{
					NPC nPC = location.doesPositionCollideWithCharacter(getBoundingBox());
					if (nPC != null)
					{
		...
		{
			Vector2 value = position.Value;
			int num = boundingBoxWidth + (damagesMonsters ? 8 : 0);
		...
		public virtual void draw(SpriteBatch b)
		{
			...
			float num = startingAlpha;
			b.Draw(spriteFromObjectSheet ? Game1.objectSpriteSheet : projectileSheet, Game1.GlobalToLocal(Game1.viewport, position + new Vector2(0f, 0f - (float)height) + new Vector2(32f, 32f)), Game1.getSourceRectForStandardTileSheet(spriteFromObjectSheet ? Game1.objectSpriteSheet : projectileSheet, currentTileSheetIndex, 16, 16), color.Value * startingAlpha, rotation, new Vector2(8f, 8f), scale, SpriteEffects.None, (position.Y + 96f) / 10000f);
			if (height.Value > 0f)
			{
				b.Draw(Game1.shadowTexture, Game1.GlobalToLocal(Game1.viewport, position + new Vector2(32f, 32f)), Game1.shadowTexture.Bounds, Color.White * num * 0.75f, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 2f, SpriteEffects.None, (position.Y - 1f) / 10000f);
			}
			for (int num2 = tail.Count - 1; num2 >= 0; num2--)
			{
				b.Draw(spriteFromObjectSheet ? Game1.objectSpriteSheet : projectileSheet, Game1.GlobalToLocal(Game1.viewport, Vector2.Lerp((num2 == tail.Count - 1) ? ((Vector2)position) : tail.ElementAt(num2 + 1), tail.ElementAt(num2), (float)tailCounter / 50f) + new Vector2(0f, 0f - (float)height) + new Vector2(32f, 32f)), Game1.getSourceRectForStandardTileSheet(spriteFromObjectSheet ? Game1.objectSpriteSheet : projectileSheet, currentTileSheetIndex, 16, 16), color.Value * num, rotation, new Vector2(8f, 8f), scale, SpriteEffects.None, (position.Y - (float)(tail.Count - num2) + 96f) / 10000f);
