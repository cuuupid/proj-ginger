	...
	public class BasicProjectile : Projectile
	{
		...
		public override void behaviorOnCollisionWithMonster(NPC n, GameLocation location)
		{
			...
			else if (spriteFromObjectSheet.Value)
			{
				...
				{
					word = Game1.objectInformation[currentTileSheetIndex.Value].Split('/')[4];
				}
				else if (Game1.objectInformation.ContainsKey(currentTileSheetIndex.Value - 1))
				{
					word = Game1.objectInformation[currentTileSheetIndex.Value - 1].Split('/')[4];
				}
				Game1.multiplayer.globalChatInfoMessage("Slingshot_Hit", ((theOneWhoFiredMe.Get(location) == null || !(theOneWhoFiredMe.Get(location) is Farmer)) ? Game1.player : (theOneWhoFiredMe.Get(location) as Farmer)).Name, (n.Name == null) ? "???" : n.Name, Lexicon.prependArticle(word));
			}
		}

		private void explosionAnimation(GameLocation location)
		{
			Rectangle sourceRectForStandardTileSheet = Game1.getSourceRectForStandardTileSheet(spriteFromObjectSheet ? Game1.objectSpriteSheet : Projectile.projectileSheet, currentTileSheetIndex);
			sourceRectForStandardTileSheet.X += 28;
			sourceRectForStandardTileSheet.Y += 28;
			...
			{
				Game1.createRadialDebris(location, "TileSheets\\Projectiles", sourceRectForStandardTileSheet, 4, (int)position.X + 32, (int)position.Y + 32, 12, (int)(position.Y / 64f) + 1);
			}
			...
			if ((bool)explode)
			{
				Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, position, flicker: false, Game1.random.NextDouble() < 0.5));
			}
