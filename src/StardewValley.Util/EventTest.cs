		...
		public void update()
		{
			if (!Game1.eventUp && !Game1.fadeToBlack)
			{
				...
				if (doingSpecifics && currentLocationIndex == -1)
				{
					...
					for (int j = 0; j < Game1.locations.Count(); j++)
					{
						...
						Dictionary<string, string> dictionary = Game1.content.Load<Dictionary<string, string>>("Data\\Events\\" + Game1.locations[j].Name);
						for (int k = 0; k < dictionary.Count; k++)
						{
							int result = -1;
							if (int.TryParse(dictionary.ElementAt(k).Key.Split('/')[0], out result) && result == Convert.ToInt32(specificEventsToDo.Last().Split(' ')[1]))
							{
								currentEventIndex = k;
								break;
							}
				...
				currentEventIndex++;
				if (!flag || currentEventIndex >= Game1.content.Load<Dictionary<string, string>>("Data\\Events\\" + locationName).Count)
				{
					currentEventIndex = 0;
			...
			if (aButtonTimer < 0)
			{
				...
				if (Game1.activeClickableMenu != null && Game1.activeClickableMenu is DialogueBox)
				{
					...
					(Game1.activeClickableMenu as DialogueBox).releaseLeftClick(Game1.graphics.GraphicsDevice.Viewport.Width / 2, Game1.graphics.GraphicsDevice.Viewport.Height - 64 - Game1.random.Next(300));
				}
			}
		}
	}
