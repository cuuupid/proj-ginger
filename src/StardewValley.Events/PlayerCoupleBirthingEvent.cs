...
namespace StardewValley.Events
{
	public class PlayerCoupleBirthingEvent : FarmEvent, INetObject<NetFields>
	{
		...
		private FarmHouse chooseHome()
		{
			...
			{
				if (Game1.getLocationFromName(item.homeLocation) is FarmHouse farmHouse && farmHouse == item.currentLocation && isSuitableHome(farmHouse))
				{
					return farmHouse;
				}
			}
			...
			{
				if (Game1.getLocationFromName(item2.homeLocation) is FarmHouse farmHouse2 && isSuitableHome(farmHouse2))
				{
					return farmHouse2;
				}
			}
