...
namespace StardewValley.Events
{
	public class FairyEvent : FarmEvent, INetObject<NetFields>
	{
		...
		public bool setUp()
		{
			...
			while (num > 0 && targetCrop.Equals(Vector2.Zero))
			{
				num--;
				if (f.terrainFeatures.Count() != 0)
				{
					KeyValuePair<Vector2, TerrainFeature> keyValuePair = f.terrainFeatures.Pairs.ElementAt(random.Next(f.terrainFeatures.Count()));
					if (keyValuePair.Value is HoeDirt && (keyValuePair.Value as HoeDirt).crop != null && !(keyValuePair.Value as HoeDirt).crop.isWildSeedCrop() && (int)(keyValuePair.Value as HoeDirt).crop.currentPhase < (keyValuePair.Value as HoeDirt).crop.phaseDays.Count - 1)
					{
						targetCrop = keyValuePair.Key;
					}
				}
			}
		...
		public void makeChangesToLocation()
		{
			...
			for (int i = (int)targetCrop.X - 2; (float)i <= targetCrop.X + 2f; i++)
			{
				for (int j = (int)targetCrop.Y - 2; (float)j <= targetCrop.Y + 2f; j++)
				{
					...
					if (f.terrainFeatures.ContainsKey(key) && f.terrainFeatures[key] is HoeDirt && (f.terrainFeatures[key] as HoeDirt).crop != null)
					{
						(f.terrainFeatures[key] as HoeDirt).crop.growCompletely();
					}
				}
			}
		}
