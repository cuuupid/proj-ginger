...
namespace StardewValley.Events
{
	public class WitchEvent : FarmEvent, INetObject<NetFields>
	{
		...
		public bool setUp()
		{
			...
			{
				if (building is Coop && !building.buildingType.Equals("Coop") && !(building.indoors.Value as AnimalHouse).isFull() && building.indoors.Value.objects.Count() < 50 && r.NextDouble() < 0.8)
				{
			...
			if (targetBuilding == null)
			{
				...
				{
					if (building2.buildingType.Equals("Slime Hutch") && building2.indoors.Value.characters.Count > 0 && r.NextDouble() < 0.5 && building2.indoors.Value.numberOfObjectsOfType(83, bigCraftable: true) == 0)
					{
		...
		public void makeChangesToLocation()
		{
			...
			{
				foreach (NPC character in targetBuilding.indoors.Value.characters)
				{
					if (character is GreenSlime)
					{
						(character as GreenSlime).color.Value = new Color(40 + r.Next(10), 40 + r.Next(10), 40 + r.Next(10));
					}
				}
				return;
			}
			for (int i = 0; i < 200; i++)
			{
				Vector2 vector = new Vector2(r.Next(2, targetBuilding.indoors.Value.Map.Layers[0].LayerWidth - 2), r.Next(2, targetBuilding.indoors.Value.Map.Layers[0].LayerHeight - 2));
				if ((targetBuilding.indoors.Value.isTileLocationTotallyClearAndPlaceable(vector) || (targetBuilding.indoors.Value.terrainFeatures.ContainsKey(vector) && targetBuilding.indoors.Value.terrainFeatures[vector] is Flooring)) && !targetBuilding.indoors.Value.objects.ContainsKey(vector))
				{
					targetBuilding.indoors.Value.objects.Add(vector, new Object(Vector2.Zero, goldenWitch ? 928 : 305, null, canBeSetDown: false, canBeGrabbed: true, isHoedirt: false, isSpawnedObject: true));
					break;
				}
			}
		}
