...
namespace StardewValley.Buildings
{
	public class Coop : Building
	{
		...
		{
			GameLocation gameLocation = new AnimalHouse("Maps\\" + nameOfIndoorsWithoutUnique, buildingType);
			...
			if (!(nameOfIndoorsWithoutUnique == "Coop2"))
			{
				...
				{
					(gameLocation as AnimalHouse).animalLimit.Value = 12;
				}
			}
			else
			{
				(gameLocation as AnimalHouse).animalLimit.Value = 8;
			}
			...
			{
				gameLocation.map.GetLayer("Front").Tiles[1, 2].TileIndex += ((Game1.player.ActiveObject.ParentSheetIndex != 180 && Game1.player.ActiveObject.ParentSheetIndex != 182) ? 1 : 2);
			}
			return gameLocation;
		}
		...
		public override void performActionOnConstruction(GameLocation location)
		{
			...
			Object @object = new Object(new Vector2(3f, 3f), 99);
			@object.fragility.Value = 2;
			indoors.Value.objects.Add(new Vector2(3f, 3f), @object);
		...
		public override void performActionOnUpgrade(GameLocation location)
		{
			...
			{
				Object @object = new Object(new Vector2(2f, 3f), 104);
				@object.fragility.Value = 2;
				indoors.Value.objects.Add(new Vector2(2f, 3f), @object);
			...
			if ((int)daysOfConstructionLeft <= 0)
			{
				if ((indoors.Value as AnimalHouse).incubatingEgg.Y > 0)
				{
					...
					if ((indoors.Value as AnimalHouse).incubatingEgg.X <= 0)
					{
						...
						FarmAnimal value = new FarmAnimal(((indoors.Value as AnimalHouse).incubatingEgg.Y == 442) ? "Duck" : (((indoors.Value as AnimalHouse).incubatingEgg.Y == 180 || (indoors.Value as AnimalHouse).incubatingEgg.Y == 182) ? "BrownChicken" : (((indoors.Value as AnimalHouse).incubatingEgg.Y == 107) ? "Dinosaur" : "Chicken")), newID, owner);
						...
						((AnimalHouse)indoors.Value).animals.Add(newID, value);
					}
				}
				if ((int)(indoors.Value as AnimalHouse).animalLimit == 16)
				{
					int num = (indoors.Value as AnimalHouse).animals.Count();
					int num2 = indoors.Value.numberOfObjectsWithName("Hay");
					int num3 = Math.Min(num - num2, (Game1.getLocationFromName("Farm") as Farm).piecesOfHay);
					(Game1.getLocationFromName("Farm") as Farm).piecesOfHay.Value -= num3;
					for (int i = 0; i < 16; i++)
					{
						if (num3 <= 0)
						{
						...
						Vector2 key = new Vector2(6 + i, 3f);
						if (!indoors.Value.objects.ContainsKey(key))
						{
							indoors.Value.objects.Add(key, new Object(178, 1));
						}
						num3--;
					}
				}
