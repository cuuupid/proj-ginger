...
namespace StardewValley.Buildings
{
	...
	public class Building : INetObject<NetFields>
	{
		...
		private readonly Action _resetTexture;
		...
		public string nameOfIndoors
		{
			get
			{
				GameLocation gameLocation = indoors.Get();
				if (gameLocation == null)
				{
					return "null";
				}
				return gameLocation.uniqueName;
			}
		}
		...
		{
			Dictionary<string, string> dictionary = Game1.content.Load<Dictionary<string, string>>("Data\\PaintData");
			if (dictionary.ContainsKey(buildingType))
			{
				...
				else if (isCabin && indoors.Value is Cabin cabin && cabin.upgradeLevel < 2)
				{
					return false;
				}
				return true;
		...
		{
			Rectangle boundingBox = new Rectangle(x, y, 1, 1);
		...
		public virtual bool doAction(Vector2 tileLocation, Farmer who)
		{
			...
			else
			{
				if (who.IsLocalPlayer && tileLocation.X == (float)(humanDoor.X + (int)tileX) && tileLocation.Y == (float)(humanDoor.Y + (int)tileY) && indoors.Value != null)
				{
					...
					Game1.warpFarmer(indoors.Value.uniqueName.Value, indoors.Value.warps[0].X, indoors.Value.warps[0].Y - 1, Game1.player.FacingDirection);
					return true;
				}
				if (who.IsLocalPlayer && buildingType.Equals("Silo") && !isTilePassable(tileLocation))
				{
					if (who.ActiveObject != null && (int)who.ActiveObject.parentSheetIndex == 178)
					{
						...
						int value = (Game1.getLocationFromName("Farm") as Farm).tryToAddHay(who.ActiveObject.Stack);
						who.ActiveObject.stack.Value = value;
						if ((int)who.ActiveObject.stack < stack)
						{
							Game1.playSound("Ship");
							DelayedAction.playSoundAfterDelay("grassyStep", 100);
							Game1.drawObjectDialogue(Game1.content.LoadString("Strings\\Buildings:AddedHay", stack - who.ActiveObject.Stack));
						}
						if (who.ActiveObject.Stack <= 0)
						{
		...
		public virtual void performActionOnBuildingPlacement()
		{
			...
			for (int i = 0; i < (int)tilesHigh; i++)
			{
				for (int j = 0; j < (int)tilesWide; j++)
				{
					Vector2 key = new Vector2((int)tileX + j, (int)tileY + i);
			...
			{
				Vector2 key2 = new Vector2((int)tileX + additionalPlacementTile.X, (int)tileY + additionalPlacementTile.Y);
				farm.terrainFeatures.Remove(key2);
			}
		}
		...
		public virtual void performActionOnConstruction(GameLocation location)
		{
			...
			{
				location.playSound("axchop");
				for (int i = tileX; i < (int)tileX + (int)tilesWide; i++)
				{
					for (int j = tileY; j < (int)tileY + (int)tilesHigh; j++)
					{
						for (int k = 0; k < 5; k++)
						{
			...
			for (int n = 0; n < getSourceRectForMenu().Width / 16 * 2; n++)
			{
				for (int num = getSourceRect().Height / 16 * 2; num >= 0; num--)
				{
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(666, 1851, 8, 8), 40f, 4, 2, new Vector2((int)tileX, (int)tileY) * 64f + new Vector2(n * 64 / 2, num * 64 / 2 - getSourceRect().Height * 4 + (int)tilesHigh * 64) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, flipped: false)
					{
						layerDepth = (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + (float)n / 10000f,
						pingPong = true,
						delayBeforeAnimationStart = (getSourceRect().Height / 16 * 2 - num) * 100,
						scale = 4f,
						alphaFade = 0.01f,
					...
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(666, 1851, 8, 8), 40f, 4, 2, new Vector2((int)tileX, (int)tileY) * 64f + new Vector2(n * 64 / 2, num * 64 / 2 - getSourceRect().Height * 4 + (int)tilesHigh * 64) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, flipped: false)
					{
						layerDepth = (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + (float)n / 10000f + 0.0001f,
						pingPong = true,
						delayBeforeAnimationStart = (getSourceRect().Height / 16 * 2 - num) * 100,
						scale = 4f,
						alphaFade = 0.01f,
		...
		public virtual void BeforeDemolish()
		{
			...
			List<Item> additionalItemsToCheckBeforeDemolish = GetAdditionalItemsToCheckBeforeDemolish();
			if (additionalItemsToCheckBeforeDemolish != null)
			{
				foreach (Item item in additionalItemsToCheckBeforeDemolish)
				{
					action(item);
				}
			}
			if (this is Mill)
			{
				foreach (Item item2 in (this as Mill).output.Value.items)
				{
					action(item2);
				}
			}
			...
			{
				foreach (Item item3 in (this as JunimoHut).output.Value.items)
				{
					action(item3);
				}
			}
			if (indoors.Value != null)
			{
				Utility.iterateAllItemsHere(indoors.Value, action);
				if (indoors.Value is Cabin)
				{
					Cellar location = Game1.getLocationFromName((indoors.Value as Cabin).GetCellarName()) as Cellar;
					Utility.iterateAllItemsHere(location, action);
				}
			}
		...
		public virtual void updateWhenFarmNotCurrentLocation(GameTime time)
		{
			if (netBuildingPaintColor.Value != null)
			{
				netBuildingPaintColor.Value.Poll(_resetTexture);
			}
		...
		public virtual void Update(GameTime time)
		{
			...
			int num = tilesHigh.Get();
			if (fadeWhenPlayerIsBehind.Value && Game1.player.GetBoundingBox().Intersects(new Rectangle(64 * (int)tileX, 64 * ((int)tileY + (-(getSourceRectForMenu().Height / 16) + num)), (int)tilesWide * 64, (getSourceRectForMenu().Height / 16 - num) * 64 + 32)))
			{
		...
		public virtual void showDestroyedAnimation(GameLocation location)
		{
			for (int i = tileX; i < (int)tileX + (int)tilesWide; i++)
			{
				for (int j = tileY; j < (int)tileY + (int)tilesHigh; j++)
				{
					location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, new Vector2(i * 64, j * 64) + new Vector2(Game1.random.Next(-16, 16), Game1.random.Next(-16, 16)), flicker: false, Game1.random.NextDouble() < 0.5)
					{
					...
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(276, 1985, 12, 11), new Vector2(i, j) * 64f + new Vector2(32f, -32f) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-16, 16)), flipped: false, 0f, Color.White)
					{
						interval = 30f,
		...
		public virtual void dayUpdate(int dayOfMonth)
		{
			...
			if ((int)daysUntilUpgrade > 0 && !Utility.isFestivalDay(dayOfMonth, Game1.currentSeason))
			{
				...
				if ((int)daysUntilUpgrade <= 0)
				{
					...
					BluePrint bluePrint = new BluePrint(getNameOfNextUpgrade());
					indoors.Value.mapPath.Value = "Maps\\" + bluePrint.mapToWarpTo;
					indoors.Value.name.Value = bluePrint.mapToWarpTo;
					buildingType.Value = bluePrint.name;
			...
			if (indoors.Value is AnimalHouse)
			{
				((AnimalHouse)(GameLocation)indoors).tapToMove.Init((AnimalHouse)(GameLocation)indoors);
			}
		}
		...
		protected virtual GameLocation getIndoors(string nameOfIndoorsWithoutUnique)
		{
			...
			{
				gameLocation = new SlimeHutch("Maps\\" + nameOfIndoorsWithoutUnique, buildingType);
			}
			else if (buildingType.Value.Equals("Shed"))
			{
				gameLocation = new Shed("Maps\\" + nameOfIndoorsWithoutUnique, buildingType);
			}
			...
			{
				gameLocation = new Cabin("Maps\\Cabin");
			}
			...
			if (gameLocation != null)
			{
				gameLocation.uniqueName.Value = nameOfIndoorsWithoutUnique + GuidHelper.NewGuid().ToString();
		...
		{
			GameLocation gameLocation = getIndoors(nameOfIndoorsWithoutUnique);
			if (gameLocation != null)
			{
				gameLocation.characters.Set(indoors.Value.characters);
				gameLocation.netObjects.MoveFrom(indoors.Value.netObjects);
				gameLocation.terrainFeatures.MoveFrom(indoors.Value.terrainFeatures);
				gameLocation.IsFarm = true;
				gameLocation.IsOutdoors = false;
				gameLocation.isStructure.Value = true;
				gameLocation.miniJukeboxCount.Set(indoors.Value.miniJukeboxCount.Value);
				gameLocation.miniJukeboxTrack.Set(indoors.Value.miniJukeboxTrack.Value);
				gameLocation.uniqueName.Value = indoors.Value.uniqueName;
				if (gameLocation.uniqueName.Value == null)
				{
					gameLocation.uniqueName.Value = nameOfIndoorsWithoutUnique + ((int)tileX * 2000 + (int)tileY);
				}
				gameLocation.numberOfSpawnedObjectsOnMap = indoors.Value.numberOfSpawnedObjectsOnMap;
				if (indoors.Value is Shed)
				{
					((Shed)gameLocation).upgradeLevel.Set(((Shed)indoors.Value).upgradeLevel.Value);
				}
				if (indoors.Value is AnimalHouse)
				{
					((AnimalHouse)gameLocation).animals.MoveFrom(((AnimalHouse)indoors.Value).animals);
					((AnimalHouse)gameLocation).animalsThatLiveHere.Set(((AnimalHouse)indoors.Value).animalsThatLiveHere);
					foreach (KeyValuePair<long, FarmAnimal> pair in ((AnimalHouse)gameLocation).animals.Pairs)
					{
				...
				if (indoors.Value != null)
				{
					gameLocation.furniture.Set(indoors.Value.furniture);
				...
				if (indoors.Value is Cabin)
				{
					Cabin cabin = gameLocation as Cabin;
				...
				if (indoors.Value != null)
				{
					gameLocation.TransferDataFromSavedLocation(indoors.Value);
				}
				indoors.Value = gameLocation;
				gameLocation = null;
				updateInteriorWarps();
				for (int num = indoors.Value.characters.Count - 1; num >= 0; num--)
				{
					NPC c = indoors.Value.characters[num];
					SaveGame.initializeCharacter(c, indoors.Value);
				}
				...
				if (indoors.Value is AnimalHouse)
				{
					...
					string text = buildingType.Value.Split(' ')[0];
					if (!(text == "Big"))
					{
						if (text == "Deluxe")
						{
		...
		public virtual bool isTilePassable(Vector2 tile)
		{
			...
			if (isCabin && flag && (int)tile.Y == (int)tileY + (int)tilesHigh - 1)
			{
				return true;
			}
			return !flag;
		...
		public virtual void drawInMenu(SpriteBatch b, int x, int y)
		{
			...
			else
			{
				int num = 108;
				int num2 = 28;
				b.Draw(texture.Value, new Vector2(x + num, y + num2), new Rectangle(getSourceRect().Width / 2 - 64, getSourceRect().Height - 136 - 2, 122, 138), color, 0f, new Vector2(0f, 0f), 4f, SpriteEffects.None, 0.89f);
			}
		}
		...
		public virtual void draw(SpriteBatch b)
		{
			...
			if (isCabin)
			{
				layerDepth = (float)(((int)tileY + ((int)tilesHigh - 1)) * 64) / 10000f;
			}
			b.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64, (int)tileY * 64 + (int)tilesHigh * 64)), getSourceRect(), color.Value * alpha, 0f, new Vector2(0f, getSourceRect().Height), 4f, SpriteEffects.None, layerDepth);
		...
		{
			Vector2 vector = ((localX == -1) ? Game1.GlobalToLocal(new Vector2((int)tileX * 64, ((int)tileY + (int)tilesHigh) * 64)) : new Vector2(localX, localY + getSourceRectForMenu().Height * 4));
			b.Draw(Game1.mouseCursors, vector, leftShadow, Color.White * ((localX == -1) ? ((float)alpha) : 1f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
			for (int i = 1; i < (int)tilesWide - 1; i++)
			{
				b.Draw(Game1.mouseCursors, vector + new Vector2(i * 64, 0f), middleShadow, Color.White * ((localX == -1) ? ((float)alpha) : 1f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
			}
			b.Draw(Game1.mouseCursors, vector + new Vector2(((int)tilesWide - 1) * 64, 0f), rightShadow, Color.White * ((localX == -1) ? ((float)alpha) : 1f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
		}
			...
			for (int i = tileX; i < (int)tileX + (int)tilesWide; i++)
			{
				for (int j = tileY; j < (int)tileY + (int)tilesHigh; j++)
				{
					if (location.isTerrainFeatureAt(i, j))
					{
						...
						if (largeTerrainFeatureAt != null && largeTerrainFeatureAt is Bush)
						{
		...
		{
			int num = Math.Min(16, Math.Max(0, (int)(16f - (float)(int)newConstructionTimer / 1000f * 16f)));
			float num2 = (float)(2000 - (int)newConstructionTimer) / 2000f;
			if ((bool)magical || (int)daysOfConstructionLeft <= 0)
			{
				drawShadow(b);
				int num3 = (int)((float)(getSourceRect().Height * 4) * (1f - num2));
				b.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64, (int)tileY * 64 + (int)tilesHigh * 64 + num3 + 4 - num3 % 4)), new Rectangle(0, getSourceRect().Bottom - (int)(num2 * (float)getSourceRect().Height), getSourceRectForMenu().Width, (int)((float)getSourceRect().Height * num2)), color.Value * alpha, 0f, new Vector2(0f, getSourceRect().Height), 4f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f);
				if ((bool)magical)
				{
					for (int i = 0; i < (int)tilesWide * 4; i++)
					{
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 + i * 16, (float)((int)tileY * 64 - getSourceRect().Height * 4 + (int)tilesHigh * 64) + (float)(getSourceRect().Height * 4) * (1f - num2))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((i % 2 == 0) ? 32 : 8)), new Rectangle(536 + ((int)newConstructionTimer + i * 4) % 56 / 8 * 8, 1945, 8, 8), (i % 2 == 1) ? (Color.Pink * alpha) : (Color.LightPink * alpha), 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
						if (i % 2 == 0)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 + i * 16, (float)((int)tileY * 64 - getSourceRect().Height * 4 + (int)tilesHigh * 64) + (float)(getSourceRect().Height * 4) * (1f - num2))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) + ((i % 2 == 0) ? 32 : 8)), new Rectangle(536 + ((int)newConstructionTimer + i * 4) % 56 / 8 * 8, 1945, 8, 8), Color.White * alpha, 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
						}
					}
					return;
				}
				...
				{
					b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 - 16 + j * 16, (float)((int)tileY * 64 - getSourceRect().Height * 4 + (int)tilesHigh * 64) + (float)(getSourceRect().Height * 4) * (1f - num2))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Rectangle(((int)newConstructionTimer + j * 20) % 304 / 38 * 64, 768, 64, 64), Color.White * alpha * ((float)(int)newConstructionTimer / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
					if (j % 2 == 0)
					{
						b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 - 16 + j * 16, (float)((int)tileY * 64 - getSourceRect().Height * 4 + (int)tilesHigh * 64) + (float)(getSourceRect().Height * 4) * (1f - num2))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Rectangle(((int)newConstructionTimer + j * 20) % 400 / 50 * 64, 2944, 64, 64), Color.White * alpha * ((float)(int)newConstructionTimer / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
					}
				}
				return;
			}
			bool flag = (int)daysOfConstructionLeft == 1;
			for (int k = tileX; k < (int)tileX + (int)tilesWide; k++)
			{
				for (int l = tileY; l < (int)tileY + (int)tilesHigh; l++)
				{
					if (k == (int)tileX + (int)tilesWide / 2 && l == (int)tileY + (int)tilesHigh - 1)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16 - 4), new Rectangle(367, 277, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(367, 309, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64 + 64 - 1) / 10000f);
					}
					else if (k == (int)tileX && l == (int)tileY)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(351, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(351, 293, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64 + 64 - 1) / 10000f);
					}
					else if (k == (int)tileX + (int)tilesWide - 1 && l == (int)tileY)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(383, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(383, 293, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64 + 64 - 1) / 10000f);
					}
					else if (k == (int)tileX + (int)tilesWide - 1 && l == (int)tileY + (int)tilesHigh - 1)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(383, 277, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(383, 325, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64) / 10000f);
					}
					else if (k == (int)tileX && l == (int)tileY + (int)tilesHigh - 1)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(351, 277, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(351, 325, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64) / 10000f);
					}
					else if (k == (int)tileX + (int)tilesWide - 1)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(383, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(383, 309, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64) / 10000f);
					}
					else if (l == (int)tileY + (int)tilesHigh - 1)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(367, 277, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(367, 325, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64) / 10000f);
					}
					else if (k == (int)tileX)
					{
						if (flag)
						...
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(351, 309, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64) / 10000f);
					}
					else if (l == (int)tileY)
					{
						if (flag)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(367, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
						}
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4) + (((int)newConstructionTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(367, 293, 16, num), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(l * 64 + 64 - 1) / 10000f);
					}
					else if (flag)
					{
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(k, l) * 64f) + new Vector2(0f, 64 - num * 4 + 16), new Rectangle(367, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
