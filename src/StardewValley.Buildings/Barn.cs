...
namespace StardewValley.Buildings
{
	public class Barn : Building
	{
		...
		{
			Point point = animalDoor.Get();
			return new Rectangle((point.X + (int)tileX) * 64, ((int)tileY + point.Y) * 64, 128, 64);
		}
		...
		public override void performActionOnUpgrade(GameLocation location)
		{
			...
			{
				Object @object = new Object(new Vector2(1f, 3f), 104);
				@object.fragility.Value = 2;
				indoors.Value.objects.Add(new Vector2(1f, 3f), @object);
			}
		}
		...
		public override void performActionOnConstruction(GameLocation location)
		{
			...
			Object @object = new Object(new Vector2(6f, 3f), 99);
			@object.fragility.Value = 2;
			indoors.Value.objects.Add(new Vector2(6f, 3f), @object);
		...
		public override void dayUpdate(int dayOfMonth)
		{
			...
			for (int i = 0; i < 16; i++)
			{
				...
				Vector2 key = new Vector2(8 + i, 3f);
		...
		public override void upgrade()
		{
			if (buildingType.Equals("Big Barn"))
			{
				...
				for (int i = 4; i < 13; i++)
				{
					indoors.Value.moveObject(16, i, 20, i);
				}
			}
			else
			{
				...
				for (int j = 6; j < 12; j++)
				{
					indoors.Value.moveObject(20, j, 23, j);
				}
