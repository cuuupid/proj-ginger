...
namespace StardewValley.TerrainFeatures
{
	public class ResourceClump : TerrainFeature
	{
		...
		public override bool performToolAction(Tool t, int damage, Vector2 tileLocation, GameLocation location)
		{
			...
			health.Value -= num;
			if (t is Axe && t.hasEnchantmentOfType<ShavingEnchantment>() && Game1.random.NextDouble() <= (double)(num / 12f) && ((int)parentSheetIndex == 602 || (int)parentSheetIndex == 600))
			{
				Debris debris = new Debris(709, new Vector2(tileLocation.X * 64f + 32f, (tileLocation.Y - 0.5f) * 64f + 32f), new Vector2(Game1.player.getStandingX(), Game1.player.getStandingY()));
				debris.Chunks[0].xVelocity.Value += (float)Game1.random.Next(-10, 11) / 10f;
				debris.chunkFinalYLevel = (int)(tileLocation.Y * 64f + 64f);
				location.debris.Add(debris);
			}
			Game1.createRadialDebris(Game1.currentLocation, debrisType, (int)tileLocation.X + Game1.random.Next((int)width / 2 + 1), (int)tileLocation.Y + Game1.random.Next((int)height / 2 + 1), Game1.random.Next(4, 9), resource: false);
			if ((float)health <= 0f)
			{
				...
				case 602:
				{
					...
					int number = (((int)parentSheetIndex == 602) ? 8 : 2);
					Random recentMultiplayerRandom;
					if (Game1.IsMultiplayer)
					{
						Game1.recentMultiplayerRandom = new Random((int)tileLocation.X * 1000 + (int)tileLocation.Y);
						recentMultiplayerRandom = Game1.recentMultiplayerRandom;
					}
					else
					{
						recentMultiplayerRandom = new Random((int)Game1.uniqueIDForThisGame + (int)Game1.stats.DaysPlayed + (int)tileLocation.X * 7 + (int)tileLocation.Y * 11);
					}
					if (t.getLastFarmerToUse().professions.Contains(12))
					{
						...
						else if (recentMultiplayerRandom.NextDouble() < 0.5)
						{
							number++;
						}
					}
					if (Game1.IsMultiplayer)
					{
						Game1.createMultipleObjectDebris(709, (int)tileLocation.X, (int)tileLocation.Y, number, t.getLastFarmerToUse().UniqueMultiplayerID);
					}
					else
					{
						Game1.createMultipleObjectDebris(709, (int)tileLocation.X, (int)tileLocation.Y, number);
					}
					...
					if (recentMultiplayerRandom.NextDouble() < 0.1)
					{
				...
				case 758:
				{
					int number = (((int)parentSheetIndex == 672) ? 15 : 10);
					if (Game1.IsMultiplayer)
					{
						Game1.recentMultiplayerRandom = new Random((int)tileLocation.X * 1000 + (int)tileLocation.Y);
					...
					if (Game1.IsMultiplayer)
					{
						Game1.createMultipleObjectDebris(390, (int)tileLocation.X, (int)tileLocation.Y, number, t.getLastFarmerToUse().UniqueMultiplayerID);
					}
					else
					{
						Game1.createRadialDebris(Game1.currentLocation, 390, (int)tileLocation.X, (int)tileLocation.Y, number, resource: false, -1, item: true);
					}
				...
				case 622:
				{
					...
					if (Game1.IsMultiplayer)
					{
						Game1.createMultipleObjectDebris(386, (int)tileLocation.X, (int)tileLocation.Y, number, t.getLastFarmerToUse().UniqueMultiplayerID);
						Game1.createMultipleObjectDebris(390, (int)tileLocation.X, (int)tileLocation.Y, number, t.getLastFarmerToUse().UniqueMultiplayerID);
						Game1.createMultipleObjectDebris(535, (int)tileLocation.X, (int)tileLocation.Y, 2, t.getLastFarmerToUse().UniqueMultiplayerID);
					}
					else
					{
						Game1.createMultipleObjectDebris(386, (int)tileLocation.X, (int)tileLocation.Y, number);
						Game1.createMultipleObjectDebris(390, (int)tileLocation.X, (int)tileLocation.Y, number);
		...
		{
			Rectangle sourceRectForStandardTileSheet = Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, parentSheetIndex, 16, 16);
			sourceRectForStandardTileSheet.Width = (int)width * 16;
			sourceRectForStandardTileSheet.Height = (int)height * 16;
			Vector2 globalPosition = tile.Value * 64f;
			if (shakeTimer > 0f)
			{
				globalPosition.X += (float)Math.Sin(Math.PI * 2.0 / (double)shakeTimer) * 4f;
			}
			spriteBatch.Draw(Game1.objectSpriteSheet, Game1.GlobalToLocal(Game1.viewport, globalPosition), sourceRectForStandardTileSheet, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (tile.Y + 1f) * 64f / 10000f + tile.X / 100000f);
		}
