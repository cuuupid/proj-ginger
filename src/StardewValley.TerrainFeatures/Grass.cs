...
namespace StardewValley.TerrainFeatures
{
	public class Grass : TerrainFeature
	{
		...
		{
			if ((byte)grassType == 1 && (!environment.GetSeasonForLocation().Equals("winter") || environment.getMapProperty("AllowGrassGrowInWinter") != null) && (int)numberOfWeeds < 4)
			{
		...
		public void setUpRandom(Vector2 tileLocation)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				offset2[i] = random.Next(-2, 3);
				offset3[i] = random.Next(-2, 3);
				offset4[i] = random.Next(-2, 3);
				flip[i] = random.NextDouble() < 0.5;
		...
		{
			if ((byte)grassType == 1 && Game1.GetSeasonForLocation(currentLocation).Equals("winter") && currentLocation.IsOutdoors && currentLocation.getMapProperty("AllowGrassSurviveInWinter") == "" && !onLoad)
			{
				return true;
			}
			if ((byte)grassType == 1)
		...
		public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation, GameLocation location)
		{
			...
			if ((t != null && t is MeleeWeapon && (int)((MeleeWeapon)t).type != 2) || explosion > 0)
			{
				...
				int num = 0;
				num = ((explosion <= 0) ? 1 : Math.Max(1, explosion + 2 - Game1.recentMultiplayerRandom.Next(2)));
				...
				numberOfWeeds.Value = (int)numberOfWeeds - num;
				...
				Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(28, tileLocation * 64f + new Vector2(Game1.random.Next(-16, 16), Game1.random.Next(-16, 16)), color, 8, Game1.random.NextDouble() < 0.5, Game1.random.Next(60, 100)));
				if ((int)numberOfWeeds <= 0)
				{
					if ((byte)grassType != 1)
					{
						Random random = (Game1.IsMultiplayer ? Game1.recentMultiplayerRandom : new Random((int)Game1.uniqueIDForThisGame + (int)tileLocation.X * 1000 + (int)tileLocation.Y * 11 + Game1.CurrentMineLevel + Game1.player.timesReachedMineBottom));
					...
					{
						Random random2 = (Game1.IsMultiplayer ? Game1.recentMultiplayerRandom : new Random((int)Game1.uniqueIDForThisGame + (int)tileLocation.X * 1000 + (int)tileLocation.Y * 11));
						double num2 = ((t.InitialParentTileIndex == 53) ? 0.75 : 0.5);
						if (random2.NextDouble() < num2 && (Game1.getLocationFromName("Farm") as Farm).tryToAddHay(1) == 0)
						{
							TemporaryAnimatedSprite temporaryAnimatedSprite = new TemporaryAnimatedSprite("Maps\\springobjects", Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 178, 16, 16), 750f, 1, 0, t.getLastFarmerToUse().Position - new Vector2(0f, 128f), flicker: false, flipped: false, t.getLastFarmerToUse().Position.Y / 10000f, 0.005f, Color.White, 4f, -0.005f, 0f, 0f);
							...
							Game1.multiplayer.broadcastSprites(t.getLastFarmerToUse().currentLocation, temporaryAnimatedSprite);
		...
		public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
		{
			...
			for (int i = 0; i < (int)numberOfWeeds; i++)
			{
				...
				spriteBatch.Draw(position: (i != 4) ? (tileLocation * 64f + new Vector2((float)(i % 2 * 64 / 2 + random.Next(-2, 2) * 4 - 4) + 30f, i / 2 * 64 / 2 + random.Next(-2, 2) * 4 + 40)) : (tileLocation * 64f + new Vector2((float)(16 + random.Next(-2, 2) * 4 - 4) + 30f, 16 + random.Next(-2, 2) * 4 + 40)), texture: texture.Value, sourceRectangle: new Rectangle(num * 15, grassSourceOffset, 15, 20), color: Color.White, rotation: shakeRotation / (float)(random.NextDouble() + 1.0), origin: Vector2.Zero, scale: scale, effects: SpriteEffects.None, layerDepth: layerDepth + (32f * scale + 300f) / 20000f);
			}
		}
		...
		public override void draw(SpriteBatch spriteBatch, Vector2 tileLocation)
		{
			for (int i = 0; i < (int)numberOfWeeds; i++)
			{
				Vector2 globalPosition = ((i != 4) ? (tileLocation * 64f + new Vector2((float)(i % 2 * 64 / 2 + offset3[i] * 4 - 4) + 30f, i / 2 * 64 / 2 + offset4[i] * 4 + 40)) : (tileLocation * 64f + new Vector2((float)(16 + offset1[i] * 4 - 4) + 30f, 16 + offset2[i] * 4 + 40)));
				spriteBatch.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, globalPosition), new Rectangle(whichWeed[i] * 15, grassSourceOffset, 15, 20), Color.White, shakeRotation / (float)(shakeRandom[i] + 1.0), new Vector2(7.5f, 17.5f), 4f, flip[i] ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (globalPosition.Y + 16f - 20f) / 10000f + globalPosition.X / 10000000f);
			}
		}
	}
}
