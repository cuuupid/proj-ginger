...
namespace StardewValley.TerrainFeatures
{
	public class HoeDirt : TerrainFeature
	{
		...
		public override bool performUseAction(Vector2 tileLocation, GameLocation location)
		{
			if (crop != null)
			{
				bool result = (int)crop.currentPhase >= crop.phaseDays.Count - 1 && (!crop.fullyGrown || (int)crop.dayOfCurrentPhase <= 0);
		...
		public bool plant(int index, int tileX, int tileY, Farmer who, bool isFertilizer, GameLocation location)
		{
			...
			if (!who.currentLocation.isOutdoors || who.currentLocation.IsGreenhouse || crop.seasonsToGrowIn.Contains(location.GetSeasonForLocation()) || who.currentLocation.SeedsIgnoreSeasonsHere())
			{
				this.crop = crop;
		...
		protected void applySpeedIncreases(Farmer who)
		{
			...
			if (!((int)fertilizer == 465 || (int)fertilizer == 466 || (int)fertilizer == 918 || who.professions.Contains(5) || flag))
			{
				return;
			}
			crop.ResetPhaseDays();
			...
			while (num3 > 0 && num4 < 3)
			{
				...
				{
					if ((j > 0 || crop.phaseDays[j] > 1) && crop.phaseDays[j] != 99999)
					{
						crop.phaseDays[j]--;
						num3--;
					}
		...
		public bool canPlantThisSeedHere(int objectIndex, int tileX, int tileY, bool isFertilizer = false)
		{
			...
			else if (this.crop == null)
			{
				...
				{
					if ((bool)crop.raisedSeeds && Utility.doesRectangleIntersectTile(Game1.player.GetBoundingBox(), tileX, tileY))
					{
						return false;
					}
					return true;
		...
		public bool paddyWaterCheck(GameLocation location, Vector2 tile_location)
		{
			...
			for (int i = -num; i <= num; i++)
			{
				...
				{
					if (location.isWaterTile((int)(tile_location.X + (float)i), (int)(tile_location.Y + (float)j)))
					{
						nearWaterForPaddy.Value = 1;
						return true;
					}
		...
		{
			if (!onLoad && !currentLocation.SeedsIgnoreSeasonsHere() && (crop == null || (bool)crop.dead || !crop.seasonsToGrowIn.Contains(Game1.currentLocation.GetSeasonForLocation())))
			{
				fertilizer.Value = 0;
			}
		...
		public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
		{
			...
			if (locationFromName.terrainFeatures.ContainsKey(key) && locationFromName.terrainFeatures[key] is HoeDirt)
			{
				b = (byte)(b + 2);
			}
			key.X -= 2f;
			...
			key.X += 1f;
			key.Y += 1f;
			if (Game1.currentLocation.terrainFeatures.ContainsKey(key) && locationFromName.terrainFeatures[key] is HoeDirt)
			{
				b = (byte)(b + 4);
			}
			key.Y -= 2f;
		...
		public void DrawOptimized(SpriteBatch dirt_batch, SpriteBatch fert_batch, SpriteBatch crop_batch, Vector2 tileLocation)
		{
			...
			if (value != 2 && (dirt_batch != null || fert_batch != null))
			{
				...
				Vector2 position = Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f, tileLocation.Y * 64f));
				if (dirt_batch != null)
				{
					dirt_batch.Draw(texture, position, new Rectangle(sourceRectPosition % 4 * 16, sourceRectPosition / 4 * 16, 16, 16), c, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-08f);
					if (value == 1)
					{
						dirt_batch.Draw(texture, position, new Rectangle(wateredRectPosition % 4 * 16 + (paddyWaterCheck(Game1.currentLocation, tileLocation) ? 128 : 64), wateredRectPosition / 4 * 16, 16, 16), c, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1.2E-08f);
					}
				}
				if (fert_batch != null)
				{
					int value2 = fertilizer.Value;
					if (value2 != 0)
					{
						Rectangle fertilizerSourceRect = GetFertilizerSourceRect(value2);
						fert_batch.Draw(Game1.mouseCursors, position, fertilizerSourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1.9E-08f);
					}
				}
			}
			if (crop != null && crop_batch != null)
			{
				crop.draw(crop_batch, tileLocation, (value == 1 && (int)crop.currentPhase == 0 && crop.shouldDrawDarkWhenWatered()) ? (new Color(180, 100, 200) * 1f) : Color.White, shakeRotation);
			}
		}
		...
		private List<Neighbor> gatherNeighbors(GameLocation loc, Vector2 tilePos)
		{
			...
			for (int i = 0; i < offsets.Length; i++)
			{
				...
				Vector2 key = tilePos + neighborLoc.Offset;
				if (terrainFeatures.TryGetValue(key, out value) && value != null && value is HoeDirt hoeDirt2 && hoeDirt2.state.Value != 2)
				{
					Neighbor item = new Neighbor(hoeDirt2, neighborLoc.Direction, neighborLoc.InvDirection);
					neighbors.Add(item);
				}
			}
			return neighbors;
		...
		public void updateNeighbors(GameLocation loc, Vector2 tilePos)
		{
			...
			foreach (Neighbor item in list)
			{
				...
				if ((int)state == 1 && (int)item.feature.state == 1)
				{
					if (item.feature.paddyWaterCheck(item.feature.currentLocation, item.feature.currentTileLocation) == paddyWaterCheck(loc, tilePos))
					{
