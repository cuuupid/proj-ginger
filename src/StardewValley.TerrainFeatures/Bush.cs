...
namespace StardewValley.TerrainFeatures
{
	public class Bush : LargeTerrainFeature
	{
		...
		{
			int num = (((int)overrideSeason == -1) ? Utility.getSeasonNumber(Game1.GetSeasonForLocation(currentLocation)) : ((int)overrideSeason));
			...
			if ((int)size == 0)
			{
				sourceRect.Value = new Rectangle(num * 16 * 2 + (int)tileSheetOffset * 16, 224, 16, 32);
			}
			else if ((int)size == 1)
			{
				if ((bool)townBush)
				...
				{
					sourceRect.Value = new Rectangle((num * 16 * 4 + (int)tileSheetOffset * 16 * 2) % texture.Value.Bounds.Width, (num * 16 * 4 + (int)tileSheetOffset * 16 * 2) / texture.Value.Bounds.Width * 3 * 16, 32, 48);
				}
			}
			else if ((int)size == 2)
			{
				if ((bool)townBush && (num == 0 || num == 1))
				{
		...
		{
			Random random = new Random((int)Game1.stats.DaysPlayed + (int)Game1.uniqueIDForThisGame + (int)tilePosition.X + (int)tilePosition.Y * 777);
			if ((int)size != 4)
			{
				if ((int)size == 1 && (int)tileSheetOffset == 0 && random.NextDouble() < 0.5 && inBloom(Game1.GetSeasonForLocation(currentLocation), Game1.dayOfMonth))
				{
					tileSheetOffset.Value = 1;
				}
		...
		{
			string text = (((int)overrideSeason == -1) ? Game1.GetSeasonForLocation(location) : Utility.getSeasonNameFromNumber(overrideSeason));
			...
			if (shakeTimer <= 0f)
			{
				if (maxShake == 0f && ((bool)greenhouseBush || (int)size != 3 || !text.Equals("winter")))
				{
		...
		private void shake(Vector2 tileLocation, bool doEvenIfStillShaking)
		{
			...
			{
				string text = (((int)overrideSeason == -1) ? Game1.GetSeasonForLocation(currentLocation) : Utility.getSeasonNameFromNumber(overrideSeason));
				int shakeOff = -1;
				if (!(text == "spring"))
				{
					if (text == "fall")
				...
				{
					int num2 = random.Next(1, 2) + Game1.player.ForagingLevel / 4;
		...
		{
			string text = (((int)overrideSeason == -1) ? Game1.GetSeasonForLocation(environment) : Utility.getSeasonNameFromNumber(overrideSeason));
			if ((int)size != 4)
			{
				if ((int)size == 1 && (int)tileSheetOffset == 0 && Game1.random.NextDouble() < 0.2 && inBloom(text, Game1.dayOfMonth))
				{
					tileSheetOffset.Value = 1;
				}
				else if (!text.Equals("summer") && !inBloom(text, Game1.dayOfMonth))
				{
					tileSheetOffset.Value = 0;
				}
				if ((int)size == 3)
				{
					tileSheetOffset.Value = (inBloom(text, Game1.dayOfMonth) ? 1 : 0);
				}
				GameLocation gameLocation = currentLocation;
		...
		public override bool seasonUpdate(bool onLoad)
		{
			...
			{
				string text = (((int)overrideSeason == -1) ? Game1.GetSeasonForLocation(currentLocation) : Utility.getSeasonNameFromNumber(overrideSeason));
				if ((int)size == 1 && text.Equals("summer") && Game1.random.NextDouble() < 0.5)
				{
					tileSheetOffset.Value = 1;
				}
				else
		...
		public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation, GameLocation location)
		{
			...
			if (t != null && t is Axe && isDestroyable(location, tileLocation))
			{
				...
				GameLocation gameLocation2 = currentLocation;
				currentLocation = location;
				shake(tileLocation, doEvenIfStillShaking: true);
				currentLocation = gameLocation2;
				if ((int)(t as Axe).upgradeLevel >= 1 || (int)size == 3)
				{
					...
					if (health <= -1f)
					{
						...
						for (int i = 0; i <= getEffectiveSize(); i++)
						{
							for (int j = 0; j < 12; j++)
							{
								Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(355, 1200 + (text.Equals("fall") ? 16 : (text.Equals("winter") ? (-16) : 0)), 16, 16), Utility.getRandomPositionInThisRectangle(getBoundingBox(), Game1.random) - new Vector2(0f, Game1.random.Next(64)), flipped: false, 0.01f, color)
								{
		...
		public override void performPlayerEntryAction(Vector2 tileLocation)
		{
			...
			string text = (((int)overrideSeason == -1) ? Game1.GetSeasonForLocation(currentLocation) : Utility.getSeasonNameFromNumber(overrideSeason));
			if (!text.Equals("winter") && !Game1.IsRainingHere(currentLocation) && Game1.isDarkOut() && Game1.random.NextDouble() < (text.Equals("summer") ? 0.08 : 0.04))
			{
