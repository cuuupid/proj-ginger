...
namespace StardewValley.Menus
{
	public class BobberBar : IClickableMenu
	{
		...
		private bool SeenTutorial
		{
			get
			{
				if ((Game1.player.fishCaught != null && Game1.player.fishCaught.Count() > 0) || Game1.currentLocation is Submarine || Game1.currentMinigame is FishingGame || Game1.isFestival())
				{
					return true;
				}
				return false;
		...
		public BobberBar(int whichFish, float fishSize, bool treasure, int bobber)
			: base(0, 0, 96, 636)
		{
			initialize(0, 0, 96, 636, showUpperRightCloseButton: true);
			mobileScale = (float)Game1.uiViewport.Height / 896f;
			mobileScale = (float)Math.Floor(mobileScale * 100f) / 100f;
			Log.It("mobileScale:" + mobileScale + " Game1.viewport.Height:" + Game1.viewport.Height);
		...
		public virtual void Reposition()
		{
			...
			int num = Game1.xEdge + 28;
			int num2 = Game1.viewport.Width - (Game1.xEdge + 176);
			if (!SeenTutorial)
			{
				num += (int)(160f / mobileScale);
				num2 -= (int)(160f / mobileScale);
			}
			xPositionOnScreen = Math.Max(num, Math.Min(num2, xPositionOnScreen));
		}

		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			if (!_closing && upperRightCloseButton.containsPoint(x, y))
			{
				_closing = true;
				emergencyShutDown();
			}
		...
		public override void update(GameTime time)
		{
			...
			else if (fadeOut)
			{
				...
				if (scale <= 0f)
				{
					scale = 0f;
					fadeOut = false;
					int num = ((Game1.player.CurrentTool != null && Game1.player.CurrentTool is FishingRod && (Game1.player.CurrentTool as FishingRod).attachments[0] != null) ? ((int)(Game1.player.CurrentTool as FishingRod).attachments[0].parentSheetIndex) : (-1));
			...
			else
			{
				...
				{
					float num2 = 548f - bobberPosition;
					float num3 = bobberPosition;
					float num4 = Math.Min(99f, difficulty + (float)Game1.random.Next(10, 45)) / 100f;
					bobberTargetPosition = bobberPosition + (float)Game1.random.Next((int)Math.Min(0f - num3, num2), (int)num2) * num4;
				}
				if (motionType == 4)
				{
				...
				{
					bobberTargetPosition = bobberPosition + (float)((Game1.random.NextDouble() < 0.5) ? SafeNext(Game1.random, -100 - (int)difficulty * 2, -51) : SafeNext(Game1.random, 50, 101 + (int)difficulty * 2));
				}
				...
				if (bobberInBar)
				{
					num5 *= ((whichBobber == 691) ? 0.3f : 0.6f);
					if (whichBobber == 691)
					{
				...
				if (treasure)
				{
					...
					if (treasureAppearTimer <= 0f)
					{
						if (treasureScale < 1f && !treasureCaught)
						{
							if (num7 > 0f)
							{
								treasurePosition = ((bobberBarPos > 274f) ? SafeNext(Game1.random, 8, (int)bobberBarPos - 20) : SafeNext(Game1.random, Math.Min(528, (int)bobberBarPos + bobberBarHeight), 500));
								Game1.playSound("dwop");
							}
							treasureScale = Math.Min(1f, treasureScale + 0.1f);
						}
						flag2 = treasurePosition + 12f <= bobberBarPos - 32f + (float)bobberBarHeight && treasurePosition - 16f >= bobberBarPos - 32f;
						if (flag2 && !treasureCaught)
						{
				...
				else if (!flag2 || treasureCaught || whichBobber != 693)
				{
					...
					float num8 = Math.Abs(bobberPosition - (bobberBarPos + (float)(bobberBarHeight / 2)));
					reelRotation -= (float)Math.PI / Math.Max(10f, 200f - num8);
					...
					if (unReelSound != null && !unReelSound.IsPlaying && !unReelSound.IsStopping && _soundsEnabled)
					{
						unReelSound.Play();
					}
				}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (!SeenTutorial)
			{
				...
				{
					b.Draw(Game1.mobileSpriteSheet, position, new Rectangle(179, 0, 48, 69), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.006f);
				}
				else
				{
					b.Draw(Game1.mouseCursors, position, new Rectangle(644, 1330, 48, 69), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.006f);
				}
			}
			if (!_closing && upperRightCloseButton != null)
			{
