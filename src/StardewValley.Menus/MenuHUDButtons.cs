...
namespace StardewValley.Menus
{
	public class MenuHUDButtons : IClickableMenu
	{
			...
			: base(Game1.uiViewport.Width / 2 + 384 + 64, Game1.uiViewport.Height - 84 - 16, 280, 84)
		{
			for (int i = 0; i < 7; i++)
			{
				buttons.Add(new ClickableComponent(new Rectangle(Game1.uiViewport.Width / 2 + 384 + 16 + i * 9 * 4, yPositionOnScreen + 20, 36, 44), i.ToString() ?? ""));
			}
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			foreach (ClickableComponent button in buttons)
			{
				...
				{
					int startingTab = Convert.ToInt32(button.name);
					Game1.activeClickableMenu = new GameMenu(startingTab);
		...
		public override void performHoverAction(int x, int y)
		{
			...
			foreach (ClickableComponent button in buttons)
			{
				if (button.containsPoint(x, y))
				{
					...
					hoverText = GameMenu.getLabelOfTabFromIndex(index);
				}
			}
		}
