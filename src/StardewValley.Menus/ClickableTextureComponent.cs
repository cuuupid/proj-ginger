	...
	public class ClickableTextureComponent : ClickableComponent
	{
		...
		public void draw(SpriteBatch b, Color c, float layerDepth, int frameOffset = 0)
		{
			...
			if (texture != null)
			{
				...
				if (drawShadow)
				{
					Utility.drawWithShadow(b, texture, new Vector2((float)bounds.X + (float)(sourceRect.Width / 2) * baseScale, (float)bounds.Y + (float)(sourceRect.Height / 2) * baseScale), value, c, 0f, new Vector2(sourceRect.Width / 2, sourceRect.Height / 2), scale, flipped: false, layerDepth);
				}
				else
				{
					b.Draw(texture, new Vector2((float)bounds.X + (float)(sourceRect.Width / 2) * baseScale, (float)bounds.Y + (float)(sourceRect.Height / 2) * baseScale), value, c, 0f, new Vector2(sourceRect.Width / 2, sourceRect.Height / 2), scale, SpriteEffects.None, layerDepth);
				}
			}
