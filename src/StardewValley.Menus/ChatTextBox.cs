...
namespace StardewValley.Menus
{
	public class ChatTextBox : TextBox
	{
		...
		{
			Task<string> task = KeyboardInput.Show(base.TitleText, "", base.Text);
			task.ContinueWith(delegate(Task<string> s)
			{
				base.Text = s.Result;
				setText(s.Result);
				PressEnter();
		...
		{
			bool flag = true;
			flag = !(Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 1000.0 < 500.0);
			...
			for (int i = 0; i < finalText.Count; i++)
			{
				...
				{
					spriteBatch.Draw(ChatBox.emojiTexture, new Vector2((float)base.X + num + 12f, base.Y + 12), new Rectangle(finalText[i].emojiIndex * 9 % ChatBox.emojiTexture.Width, finalText[i].emojiIndex * 9 / ChatBox.emojiTexture.Width * 9, 9, 9), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
				}
				else if (finalText[i].message != null)
				{
					spriteBatch.DrawString(ChatBox.messageFont(LocalizedContentManager.CurrentLanguageCode), finalText[i].message, new Vector2((float)base.X + num + 12f, base.Y + 12), ChatMessage.getColorFromName(Game1.player.defaultChatColor), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.99f);
				}
				num += finalText[i].myLength;
			}
		}
	}
}
